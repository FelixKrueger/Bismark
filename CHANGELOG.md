...
- **Bowtie 2 `--local` MAPQ now matches Bowtie 2's own normalization ([#1079](https://github.com/FelixKrueger/Bismark/issues/1079)).** MAPQ is normalized by the score range a valid alignment can span. Bismark computed that as `abs(score_min)`, which assumes the best possible alignment score is 0 — true end-to-end, but wrong in Bowtie 2 `--local`, where a match earns `--ma` (default 2) per base and the perfect score is `2 × read_length`. Because `score_min` grows logarithmically while the perfect score grows linearly, the denominator was far too small and local MAPQ saturated: a 250 bp read aligning at only 30 % of its perfect score still scored 44 (Bowtie 2 gives 22), so `-q` filters were close to no-ops in local mode. Bowtie 2's `max(1, perfect_score − score_min)` is now used, for single- and paired-end (both mates summed). **Local-mode MAPQ values therefore differ from Bismark 3.1.0 and Perl v0.25.1** — the previous values were a faithful port of a Perl bug. Expect MAPQ values you have not seen before in local output: the ladder rungs that require `best_over == diff` (39/35/34/33/32/31) now mean "the alignment scored exactly the perfect score", which was unreachable while the denominator was `abs(score_min)`. The **default end-to-end path is unaffected and stays byte-identical** — it keeps `abs(score_min)` untouched, because the new denominator is applied only where the perfect score is nonzero. (The two expressions are *not* equivalent in general — `max(1, −score_min)` differs from `abs(score_min)` whenever `score_min > −1`, e.g. for a non-default `--score_min` — which is why it is deliberately not applied everywhere.) HISAT2 `--local` likewise keeps its denominator, though its score-min *form* does change; see the next entry. Reported and diagnosed by @9xg.
- **HISAT2 `--local` MAPQ now matches what HISAT2 itself would compute ([#1079](https://github.com/FelixKrueger/Bismark/issues/1079), [#1080](https://github.com/FelixKrueger/Bismark/issues/1080)).** Two things were wrong. First, Bismark passes HISAT2 the linear `--score-min L,0,-0.2` but recomputed `score_min` logarithmically when deriving MAPQ, evaluating a threshold HISAT2 never applied (`-0.92` instead of `-20` at 100 bp); the emitted form is now the single source of truth for both. Second, Bismark applied Bowtie 2's *local* MAPQ ladder, whereas HISAT2 — like Bowtie 2 — selects the ladder from whether scoring is monotone, and HISAT2's scoring always is: it has no local mode of its own and forces its match bonus to 0. So HISAT2 `--local` now uses the **end-to-end** ladder, putting its MAPQ on the same scale as HISAT2 end-to-end instead of capping at 44 for no scoring reason. **HISAT2 `--local` MAPQ values therefore change** (e.g. a 50 bp read with `AS:i:-1` and no second-best goes from 22 to 42). Its perfect alignment score is confirmed to be 0 — previously documented as "not exactly known" — so this is no longer provisional.
