...
- **New `--five_base_emit_multiplicity {duplex|simplex|both}`: emit simplex (single-strand) consensus molecules ([#1104](https://github.com/FelixKrueger/Bismark/issues/1104)).** `--five_base_consensus` collapses only molecules seen from both strands; single-strand families were counted and discarded, which on cfDNA is most of the library. `simplex`/`both` now write them to `<out>.5base_simplex.bam`, one record per family on its own strand, and tag every consensus record `mx:i` (2 = duplex, 1 = simplex) so the classes survive a `samtools merge`. A simplex family still holds every PCR copy of its strand, so the collapse deduplicates and error-corrects, but it has **no cross-strand variant check** — a genuine `C>T` reads as methylation, so treat simplex output as lower confidence and pair it with `--five_base_deconvolution`. Default (`duplex`) output is unchanged. Requested by @Danielsm8.
