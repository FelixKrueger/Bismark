...
- **The converted reads are now streamed to the aligner instead of written to disk ([#1120](https://github.com/FelixKrueger/Bismark/issues/1120)).** Bismark converts your reads in silico before aligning them (`C→T`, plus `G→A`), and every run wrote that converted set to `--temp_dir` first — typically about **5x the size of a gzipped input**, and up to four such files for a non-directional paired-end run. On a disk-constrained scratch that was the binding constraint: not slow, but unable to start at all. `--gzip` took roughly 5x off it and never took it to zero. Those bytes have exactly one consumer, the aligner — the methylation call re-reads your *original* FastQ — so they are now handed to the aligner through named pipes and **never touch the disk**. The converted footprint of a default run is zero — measured at **4 KB against 96 MB** for a 200,000-pair run whose input was 93 MB uncompressed. Wall time is unchanged: conversion no longer has to finish before the aligner is spawned, but on a fast converter that serial phase was small, and the fan-out costs about as much as it saves.

  Output is **byte-identical**, by construction rather than by argument: the aligner receives the same bytes from the same single conversion routine, and Bismark writes its own `@PG` header, so the one field that records the input path never reaches your BAM. Pass **`--no_stream_converted`** to write files as before, to inspect the converted reads or to bisect a suspected problem. `--gzip` no longer has anything to compress on the default path and is inert there.

  **`--rammap`'s in-process backend goes further and uses no intermediate at all.** It aligns inside the Bismark process, so pointing it at a temp file meant the converter serialised records to disk purely so that the same process could parse them straight back. It now receives the converted reads in memory, with neither a file nor a pipe in between. The external-binary backend (`--rammap_subprocess`) keeps files.

  Runs that cannot stream fall back to files on their own and say why, on STDERR: **`--hisat2`** (HISAT2 rejects a named pipe as its read input — its only disk lever stays `--gzip`), the **`--combined_index`** alignment models, **`--rammap`**, and a `--temp_dir` on a filesystem that will not host a FIFO. `--illumina_5base` aligns against the unconverted genome and writes no converted reads at all. `--multicore N` still writes its per-chunk input subsets; only the converted reads are streamed.

- **New `--five_base_emit_multiplicity {duplex|simplex|both}`: emit simplex (single-strand) consensus molecules ([#1104](https://github.com/FelixKrueger/Bismark/issues/1104)).** `--five_base_consensus` collapses only molecules seen from both strands; single-strand families were counted and discarded, which on cfDNA is most of the library. `simplex`/`both` now write them to `<out>.5base_simplex.bam`, one record per family on its own strand, and tag every consensus record `mx:i` (2 = duplex, 1 = simplex) so the classes survive a `samtools merge`. A simplex family still holds every PCR copy of its strand, so the collapse deduplicates and error-corrects, but it has **no cross-strand variant check** — a genuine `C>T` reads as methylation, so treat simplex output as lower confidence and pair it with `--five_base_deconvolution`. In `simplex` mode the run report also names the duplex families it excluded, because that mode prints no duplex line of its own. Default (`duplex`) output is unchanged. Requested by @Danielsm8.
- The 5-Base consensus BAM's record order is now deterministic (sorted by family span); it previously followed hash-map iteration order, which varied from run to run ([#1104](https://github.com/FelixKrueger/Bismark/issues/1104)).
- **`--five_base_index` is validated before the run starts ([#1100](https://github.com/FelixKrueger/Bismark/issues/1100)).** A 5-Base run whose `--five_base_index` did not exist was accepted all the way into alignment: the aligner's own complaint scrolled past, Bismark then reported *"produced fewer PE records than read pairs (desync)"* — describing a symptom rather than the cause — and a truncated BAM plus a report were left behind in the output directory. The index is now checked at start-up and a missing or incomplete one fails immediately, naming the flag and the files it looked for, with nothing written — not even the output directory. The check mirrors what the aligners themselves accept rather than second-guessing them: both index sizes (a mammalian bowtie2 index is `.bt2l`, hisat2's `.ht2l`), the full file set for the chosen aligner, and — exactly as bowtie2's and hisat2's `adjustEbwtBase` does — a retry under `$BOWTIE2_INDEXES` / `$HISAT2_INDEXES` when no `<basename>.1.<ext>` can be opened, including when the basename contains a path separator. That last condition matters: the aligners' *wrapper scripts* consult a wider file glob than the binaries that actually decide, so gating on it would have rejected setups the aligner runs happily. A **bisulfite-converted** basename (`Bisulfite_Genome/CT_conversion/BS_CT` or its `GA` twin) is now rejected by name too: 5-Base reads are unconverted, so aligning them against a converted index inverted every methylation call and previously did so silently. **One configuration that used to work will now be rejected:** a *stub or wrapper* aligner supplied via `--path_to_bowtie2` that does not need a real index on disk — a conda shim, a cluster wrapper, or a script that stages an index at run time. Pass a real index basename, or one whose files exist. Nothing changes for a run whose index is present, and the bisulfite routes are untouched.
- **`--output_dir` is created if it does not exist, for every run, not just 5-Base.** Previously the run printed its whole resolved-configuration summary and then died at `failed to open BAM … No such file or directory`. Perl Bismark creates the directory and announces it, and that behaviour is restored, notice included. Deliberately unlike Perl, missing *parent* directories are created too rather than the run failing, matching the rest of the suite; the notice is what keeps a mistyped path visible. `--temp_dir` was already auto-created.
- **`--illumina_5base` no longer demands the bisulfite-converted indexes it never opens ([#1099](https://github.com/FelixKrueger/Bismark/issues/1099)).** 5-Base reads keep full complexity, so they align to a plain unconverted index supplied with `--five_base_index`, or — on the default minimap2 route — to the genome FASTA directly. Neither path ever reads the `C→T`/`G→A` indexes under `Bisulfite_Genome/`, but the genome check required them anyway, so a 5-Base run against a raw genome folder was rejected with *"the Bowtie 2 index of the C->T->converted genome seems to be faulty or non-existant ('BS_CT.1.bt2l'). Please run the bismark_genome_preparation before running Bismark"* — advice that would have spent hours building indexes the run then ignores. **The default route was hit even harder:** it asked for `BS_CT.mmi`, which `bismark_genome_preparation` writes only under `--minimap2`, so a genome prepared the ordinary Bowtie 2 way was *also* refused. A 5-Base run now needs only the genome FASTA. `--genome` is still required and still has to contain that FASTA — the reference is what methylation is called against — and a genome folder with no FASTA still fails, now saying so. The resolved-configuration summary a 5-Base run prints reports the `--five_base_index` in place of the two converted-index basenames and the large-index flag, none of which are probed on this path. **Nothing changes for any bisulfite run:** an unprepared genome is still rejected with the same error, which is covered by a test that must fail if this branch is ever made unconditional. Also fixed alongside: a paired-end 5-Base desync error hard-coded the aligner name, so a `--bowtie2` run reported `minimap2 produced fewer PE records than read pairs`. Reported by @Danielsm8.
- **New `--five_base_bisulfite_bam`: re-encode a 5-Base BAM into bisulfite convention, for tools that read `SEQ` instead of `XM` ([#1095](https://github.com/FelixKrueger/Bismark/issues/1095), [#787](https://github.com/FelixKrueger/Bismark/issues/787)).** Some downstream tools take methylation out of the BAM's `SEQ` rather than out of the `XM` tag — [`wgbs_tools`](https://github.com/nloyfer/wgbs_tools)' `bam2pat` (the input to `UXM_deconv` cell-of-origin deconvolution) compares the read base at each CpG against the reference, which is why it needs a genome at all. 5-Base `SEQ` holds the raw read and 5-Base chemistry is bisulfite's inverse, so those tools produce **systematically inverted** calls on 5-Base data: not noisy, inverted. Measured here, stock `bam2pat` on a fully CpG-methylated 5-Base control reports **0.0 %** methylation; after this conversion the same unmodified binary reports **100.0 %**, matching Bismark's own calls. The new flag reads one or more existing 5-Base BAMs and writes `<stem>.bisulfite.bam`, setting each called cytosine to the base bisulfite chemistry would have produced (`C`/`G` when `XM` says methylated, `T`/`A` when not; which pair applies follows from `XG`). It needs **no genome** and no re-alignment — the reference is reconstructed per record from `MD`, and that reconstruction is *proved* against the record's own `NM`/`MD` before anything is emitted — and it leaves `XM` untouched, so the output is still readable by Bismark's own extractor and by `bam2pat --ds_test`. `NM`/`MD` are recomputed exactly, preserving Bismark's non-standard treatment of soft-clipped bases as mismatches rather than silently "fixing" it. Positions the alignment run masked via `--five_base_baseq` keep the raw 5-Base base in `SEQ` and would otherwise be scored inverted in the output too; they are masked to `N` instead, identified from the reconstructed reference so no threshold or header parsing is involved. **This is never the primary BAM:** its `SEQ` no longer matches the sequencer, and because 5-Base cannot separate a genuine `C>T` from 5mC without the opposite strand, real variants are encoded as methylation — harmless for fragment-level CpG tools, but it must not feed a variant caller. Output is read-order; `bam2pat` refuses a BAM that is not `SO:coordinate`, so the run prints the `samtools sort`/`index` commands and warns that the chromosome naming must match your `wgbstools init_genome`. Each run reports a **flip rate**, which is exactly `1.000` for 5-Base input and `0.000` for bisulfite input because every called position necessarily changes under inversion; anything in between fails the run rather than writing a half-converted file. Converting an ordinary bisulfite BAM is therefore a no-op by construction, and that identity is a CI gate. Requires `--illumina_5base`; mutually exclusive with `--five_base_consensus_from_bam`. Nothing changes for any existing run. Requested by @Danielsm8.
- **`--rammap` now honours `--mm2_short_reads` / `--mm2_pacbio` ([#1092](https://github.com/FelixKrueger/Bismark/issues/1092)).** `--rammap` defaults to the bundled in-process backend, and that backend hard-coded the `map-ont` preset — so `--rammap --mm2_short_reads` silently ran `map-ont` while `--rammap_subprocess --mm2_short_reads` correctly ran `sr`. The flag was accepted and had no effect. The preset is now resolved once and used by both the emitted option string and the in-process backend, so they cannot disagree. **`--rammap --mm2_short_reads` therefore produces different alignments than in 3.1.0** — that is the flag finally taking effect. Because `sr` changes not only the mismatch and gap penalties but also the chain/DP thresholds, **mapping rate changes too, not just alignment scores**: some reads that mapped under `map-ont` will not map under `sr` and vice versa, so mapping-efficiency percentages, `--unmapped` output and the unique/ambiguous split all shift. In-process `sr` is newly reachable and its concordance against the subprocess and against minimap2 is **not** yet measured (the existing rammap concordance figures were all established on `map-ont`); it is also a hybrid, applying `sr` mapping options over an index built with the genome-preparation `k`, because a pre-built index overrides a preset's index parameters. Separately, `--mm2_pacbio` differs from the default in those index-build parameters plus one chaining coefficient derived from them, so against a pre-built index — which supplies its own — it is near-inert on every backend — the run now says so rather than leaving it to be inferred. The run's backend notice names the resolved preset. Nothing changes for a plain `--rammap` run, for `--rammap_subprocess`, or for `--minimap2`.
- **Bowtie 2 `--local` MAPQ now matches Bowtie 2's own normalization ([#1079](https://github.com/FelixKrueger/Bismark/issues/1079)).** MAPQ is normalized by the score range a valid alignment can span. Bismark computed that as `abs(score_min)`, which assumes the best possible alignment score is 0 — true end-to-end, but wrong in Bowtie 2 `--local`, where a match earns `--ma` (default 2) per base and the perfect score is `2 × read_length`. Because `score_min` grows logarithmically while the perfect score grows linearly, the denominator was far too small and local MAPQ saturated: a 250 bp read aligning at only 30 % of its perfect score still scored 44 (Bowtie 2 gives 22), so `-q` filters were close to no-ops in local mode. Bowtie 2's `max(1, perfect_score − score_min)` is now used, for single- and paired-end (both mates summed). **Local-mode MAPQ values therefore differ from Bismark 3.1.0 and Perl v0.25.1** — the previous values were a faithful port of a Perl bug. Expect MAPQ values you have not seen before in local output: the ladder rungs that require `best_over == diff` (35/34/33/32/31) now mean "the alignment scored exactly the perfect score", which was unreachable while the denominator was `abs(score_min)`. The **Bowtie 2 and HISAT2 end-to-end paths are unaffected and stay byte-identical** — they keep `abs(score_min)` untouched, because the new denominator is applied only where the perfect score is nonzero. (The two expressions are *not* equivalent in general — `max(1, −score_min)` differs from `abs(score_min)` whenever `score_min > −1`, e.g. for a non-default `--score_min` — which is why it is deliberately not applied everywhere.) minimap2/rammap also have a nonzero perfect score and are covered by their own entry below. HISAT2 `--local` likewise keeps its denominator, though its score-min *form* does change; see the next entry. Reported and diagnosed by @9xg.
- **HISAT2 `--local` now uses the MAPQ ladder HISAT2 itself would select ([#1079](https://github.com/FelixKrueger/Bismark/issues/1079), [#1080](https://github.com/FelixKrueger/Bismark/issues/1080)).** Two things were wrong. First, Bismark passes HISAT2 the linear `--score-min L,0,-0.2` but recomputed `score_min` logarithmically when deriving MAPQ, evaluating a threshold HISAT2 never applied (`-0.92` instead of `-20` at 100 bp); the emitted form is now the single source of truth for both. Second, Bismark applied Bowtie 2's *local* MAPQ ladder, whereas HISAT2 — like Bowtie 2 — selects the ladder from whether scoring is monotone, and HISAT2's scoring always is: it exposes no local mode of its own and forces its match bonus to 0. So HISAT2 `--local` now uses the **end-to-end** ladder, putting its MAPQ on the same scale as HISAT2 end-to-end instead of capping at 44 for no scoring reason. **HISAT2 `--local` MAPQ values therefore change** (e.g. a 50 bp read with `AS:i:-1` and no second-best goes from 22 to 42). **The floor moves the same way, and matters more:** a uniquely-aligned read that scored poorly used to bottom out at 22 and can now be **0**, so MAPQ/`-q` filters downstream of `--hisat2 --local` will drop reads they previously kept. Its perfect alignment score is confirmed to be 0 — previously documented as "not exactly known" — so the ladder choice is no longer a guess. (Bismark still recomputes MAPQ across its 2–4 strand instances rather than reading HISAT2's value, so this is consistency with HISAT2's score regime, not a value-for-value match with HISAT2's own MAPQ.)
- **minimap2 and rammap MAPQ is no longer near-constant ([#1081](https://github.com/FelixKrueger/Bismark/issues/1081)).** Both aligners report **positive** alignment scores, but Bismark normalized them by `abs(score_min)` — a denominator that assumes the best possible score is 0. The ratio the MAPQ ladder consumes was therefore always above 1, so the top rung always won: **MAPQ was 42 for every uniquely-aligned minimap2 read that had no competing alignment from another strand instance** — the large majority — at every read length and every alignment score, and MAPQ/`-q` filters were close to no-ops. The best possible score is `2 × read_length` (every preset Bismark can select awards 2 per matching base), and that is now the denominator. Such reads move from a flat 42 to **44 / 44 / 41 / 28 / 22** as the alignment score falls from 100 % to 20 % of the best possible score.

  Reads that *do* have a competing alignment are scored on the ladder's second-best sub-rungs, and they move in **both** directions: a near-tie (say 200 vs 190 at 100 bp) drops from 25 to **11**, while a clear winner (200 vs 20) rises from 33 to **39**. Precisely: this branch is reached when a **later** strand instance strictly out-scores every earlier one, which for directional single-end data is roughly half the reads that align to both converted genomes — so whether a given read moves this way depends on which strand won. **Those reads can now score as low as 2.** That is the fix working, not a regression: a read matching two strand instances almost equally is not high-confidence, and 25 overstated it. Values below 22 were already reachable on this branch before the change (6, 12, 17, 18); what grows, by roughly an order of magnitude, is how many reads land there. So **`-q`-filtered read counts downstream of `--minimap2`/`--rammap` do change** — review your filtering before re-running an existing pipeline. A *uniquely*-aligned read with no competing alignment still cannot fall below 22 (the alternative ladder would have floored it at 0), so for that majority a `-q 20` filter remains close to a no-op.
...
- **Illumina 5-Base (5mC→T) deconvolution / duplex / consensus graduated to GA-supported (#787 / #1061).** These modes shed their EXPERIMENTAL/PREVIEW labels: they are GA-supported under a concordance contract (NOT byte-identical — there is no Perl 5-Base oracle), validated against Illumina DRAGEN v4.4.6 on two real samples (NA12878 + HG002) plus a reproducible lambda/pUC19 spike-in CI gate. `--five_base_umi_qname` stays preview. CLI-label + docs change; the faithful bisulfite/EM-seq suite stays byte-frozen.
