...
- **HISAT2 `--local` now evaluates the score-min function it is actually given.** Bismark passes HISAT2 the linear `--score-min L,0,-0.2`, but recomputed `score_min` logarithmically when deriving MAPQ — evaluating a threshold HISAT2 never applied (`-0.92` instead of `-20` at 100 bp). The emitted form is now the single source of truth for both, **so HISAT2 `--local` MAPQ values change** (e.g. a 50 bp read with `AS:i:-1` and no second-best goes from 22 to 44). Its perfect score is still treated as 0: Bismark's own `--local` documentation notes that for HISAT2 "it is currently not exactly known how the best alignment is calculated", so HISAT2-local MAPQ is now self-consistent but is **not** yet claimed to match a HISAT2 reference — tracked in [#1080](https://github.com/FelixKrueger/Bismark/issues/1080).
