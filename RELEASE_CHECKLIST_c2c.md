# `bismark-coverage2cytosine` — v1.0 release checklist (Phase E)

The real-data byte-identity gate. A clean run of `scripts/c2c_byte_identity_matrix.sh`
(exit 0) on **oxy** is the gate for the **`bismark-coverage2cytosine-v1.0`** tag.

Design: `plans/05292026_bismark-coverage2cytosine/phase-e-byte-identity-gate/PLAN.md` (rev 1).
Machine: **oxy** (Felix directive 2026-05-30; see SPEC §12.3, `reference_colossal_access` memory).
...
# V12 — full matrix on the fixture must PASS (exit 0):
...
cd ~/Github/Bismark && git fetch && git checkout rust/coverage2cytosine && git pull --ff-only
...
The 9 cells: `cx` (`--CX --gzip`), `default`, `zero` (`--zero_based`), `gzip`, `thr`
...
```sh
git tag -a bismark-coverage2cytosine-v1.0 -m "coverage2cytosine Rust port v1.0 — \
byte-identical to Perl v0.25.1 across the Phase E matrix on oxy (<dataset>, <date>)."
git push origin bismark-coverage2cytosine-v1.0
```
Then: update PR #892 + epic #891 (Phase E ☑), and note the gate result + perf table in the PR.

## 7. What this gate proves

Rust `coverage2cytosine_rs` reproduces Perl `coverage2cytosine` v0.25.1 byte-for-byte on real
full-genome data, across CpG/CX/zero/threshold/gzip/split/merge/discordance. Because the cov input
