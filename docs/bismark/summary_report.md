...
Basename of the output file (optional). Generate a text file with all relevant extracted values '_basename_.txt') as well as an HTML report ('_basename_.html'). If not specified explicitly, the basename is '_bismark_summary_report_'.

- `--title <string>`

Optional HTML report title; use `--title "speech marks for text with spaces"`. Default: '_Bismark Summary Report_'.
