...
For the latest development build instead of a pinned release, replace `--tag bismark-rust-v2.0.0-beta.12` with `--branch rust/iron-chancellor`. **Updating:** re-run the `--branch` command and cargo picks up the newest commit automatically; to move between pinned releases, bump the `--tag` (re-running the same `--tag` is a no-op — add `--force` to reinstall in place).
