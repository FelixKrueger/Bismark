...
docker pull ghcr.io/felixkrueger/bismark:2.0.0-beta.4  # pinned
```
...
cargo install --git https://github.com/FelixKrueger/Bismark \
  --tag bismark-rust-v2.0.0-beta.4 --locked \
...
For the latest development build instead of a pinned release, replace `--tag bismark-rust-v2.0.0-beta.4` with `--branch rust/iron-chancellor`. **Updating:** re-run the `--branch` command and cargo picks up the newest commit automatically; to move between pinned releases, bump the `--tag` (re-running the same `--tag` is a no-op — add `--force` to reinstall in place).
