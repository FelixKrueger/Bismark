...
/// Bismark MAPQ for an end-to-end alignment. `read2_len` is `Some` only for
/// paired-end (Phase 7); single-end passes `None`. `intercept`/`slope` are the
/// `--score_min` parameters (default `0.0` / `-0.2`).
...
pub fn calc_mapq(
    ...
    // scMin = intercept + slope·readLen (end-to-end); add read 2 for PE (3932–36).
    let mut sc_min = intercept + slope * read1_len as f64;
    if let Some(l2) = read2_len {
        sc_min += intercept + slope * l2 as f64;
    }
...
mod tests {
    ...
    fn no_second_best_ladder() {
        ...
        assert_eq!(calc_mapq(50, None, 0, None, I, S), 42); // bestOver 10 = diff (>=0.8)
        assert_eq!(calc_mapq(50, None, -3, None, I, S), 40); // 7 = 0.7·diff
        assert_eq!(calc_mapq(50, None, -4, None, I, S), 24); // 6 = 0.6·diff
        assert_eq!(calc_mapq(50, None, -5, None, I, S), 23); // 5 = 0.5·diff
        assert_eq!(calc_mapq(50, None, -6, None, I, S), 8); // 4 = 0.4·diff
        assert_eq!(calc_mapq(50, None, -7, None, I, S), 3); // 3 = 0.3·diff
        assert_eq!(calc_mapq(50, None, -10, None, I, S), 0); // 0
    }

    #[test]
    fn with_second_best_top_buckets() {
        // as_best 0 (bestOver 10 == diff), vary second-best.
        assert_eq!(calc_mapq(50, None, 0, Some(-10), I, S), 39); // bestDiff 10 (>=0.9), ==diff
        assert_eq!(calc_mapq(50, None, 0, Some(-8), I, S), 38); // bestDiff 8 (>=0.8), ==diff
        assert_eq!(calc_mapq(50, None, 0, Some(-5), I, S), 35); // bestDiff 5 (>=0.5), ==diff
    }

    #[test]
    fn with_second_best_not_at_diff() {
        // as_best -3 (bestOver 7, not == diff 10), second-best near.
        assert_eq!(calc_mapq(50, None, -3, Some(-3), I, S), 1); // bestDiff 0 → else; 7>=6.7
        assert_eq!(calc_mapq(50, None, -3, Some(-10), I, S), 26); // bestDiff 7 = 0.7·diff, not ==diff
        assert_eq!(calc_mapq(50, None, -3, Some(-13), I, S), 33); // bestDiff 10 ≥ 0.9·diff, not ==diff
    }

    #[test]
    fn non_integer_scmin() {
        // readLen 51 → scMin -10.2, diff 10.2; as_best 0 → bestOver 10.2 >= 8.16 → 42.
        assert_eq!(calc_mapq(51, None, 0, None, I, S), 42);
    }
    ...
    fn inner_threshold_leaves_pinned() {
        ...
        for &(len, ab, asb, want) in cases {
            let got = calc_mapq(len, None, ab, Some(asb), I, S);
    ...
    fn user_score_min_slope() {
        ...
        assert_eq!(calc_mapq(50, None, 0, None, 0.0, -0.4), 42); // bestOver 20 = diff
        assert_eq!(calc_mapq(50, None, -6, None, 0.0, -0.4), 40); // bestOver 14 = 0.7·20
    }
}
