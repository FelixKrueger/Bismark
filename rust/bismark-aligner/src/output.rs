...
pub fn write_raw_pe_ambig_lines<W: std::io::Write>(
    ...
    // Strip the read-number tag from the QNAME (Perl 3677-3678). Bowtie 2 / HISAT2 clip
    // the outer `/1`,`/2` so their tail is a single `/1`,`/2`; minimap2 lines are
    // normalised to the same single-suffix shape by `Minimap2PairedStream`, so this single
    // strip is correct for every backend.
