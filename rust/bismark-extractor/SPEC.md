...
- **Mixed-metric differential check at N=1** (the `overlap` cell's `--include_overlap` adds counts at existing M-bias positions rather than new rows — row count is unchanged; count-sum strictly increases):
  ...
  - `overlap`: M-bias data **count-sum** (`sum(methylated + unmethylated)` across all R2 data rows) **strictly > D's same metric**. (rev 2, 2026-05-29: the original `≥ 5%` floor was over-specified — the `--include_overlap` count bump scales with the per-library mate-overlap-base fraction, not a fixed constant; the always-true invariant is monotonic `> D`. The 80%-properly-paired pre-flight gate ensures overlap is meaningful. SRR24827378_10M measured +2.28%, byte-identical Perl≡Rust.)
