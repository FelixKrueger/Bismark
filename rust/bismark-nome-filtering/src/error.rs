...
//! propagate from the promoted [`bismark_io::genome`] reader via the
//! [`BismarkNomeError::Genome`] `#[from]` variant — note these come from the
//! module-local `GenomeError`, NOT from `bismark_io::BismarkIoError` (keeping
...
pub enum BismarkNomeError {
    /// Genome-load failure from the promoted `bismark_io::genome` reader.
    #[error(transparent)]
