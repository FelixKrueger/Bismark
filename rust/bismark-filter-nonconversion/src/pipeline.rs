...
//!
//! ## Why raw `RecordBuf` (not `bismark_io::BismarkRecord`)
//!
...
use crate::filename;
use crate::filter::{FilterMode, read_fails};
...
/// the last file, by [`crate::run`]). Returns the [`FilterReport`] for the
...
pub fn filter_one(
    ...
    let is_paired = match explicit_mode {
        Some(p) => p,
        None => bismark_io::detect_paired_from_header(&header).ok_or_else(|| {
