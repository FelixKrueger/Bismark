...
| `bismark` (aligner) | `bismark-aligner` (`bismark_rs`) | 1.0.0-alpha.1 | 🚧 **Phases 1–9 of 10 merged** (#930 = Ph 1–8, #942 = Ph 9a FastA, + Ph 9b threading). Bowtie 2 backend, **SE + PE, FastQ + FastA, all 3 library types (directional / non-directional / pbat)** — read-conversion → 2–4 instances → lockstep merge/scoring/MAPQ → `XM`/`XR`/`XG` → BAM + report + `--unmapped`/`--ambiguous`/`--ambig_bam`, **byte-identical** to Perl v0.25.1 + Bowtie 2 2.5.5 (oxy, 1M reads/pairs). **Order-preserving `--multicore`/`--parallel`** (worker-count-invariant: `--parallel N` byte-identical to `--parallel 1` and Perl single-core). The ~74% runtime "big beast". Remaining: full-scale real-data gate (Ph 10); HISAT2/minimap2 = v1.x |
...
- 🚧 **Next:** the `bismark` aligner **Phase 10** (full-scale real-data gate) — on `rust/aligner-v1`. Phases 1–9 (SE + PE, FastQ + FastA, all library types, + order-preserving threading) are byte-identical and merged.
