...
| `bismark` (aligner) | `bismark-aligner` (`bismark_rs`) | 1.0.0-alpha.1 | In progress — Phases 1–8/10: **all library types (directional / non-directional / pbat), SE + PE, FastQ** complete (CLI → C→T/G→A conversion → 2–4 Bowtie 2 instances → lockstep merge/scoring/MAPQ → genomic-seq + `XM`/`XR`/`XG` → BAM + alignment report + `--unmapped`/`--ambiguous`/`--ambig_bam`), **byte-identical** to Perl v0.25.1 + Bowtie 2 2.5.5 on oxy (all 4 mode×layout cells, 1M reads/pairs). FastA / threading remain |
