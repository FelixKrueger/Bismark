...
- 🚧 **Now: `bismark` aligner v1.x — HISAT2 + minimap2 backends** (epic `plans/06052026_bismark-aligner-v1x/`, branch `rust/aligner-v1x`). Spike-first byte-identity per aligner; HISAT2 first (determinism spike → wrapper), then minimap2 (wrapper + merge adaptation for its both-strand alignment). Then v2 (rammap / combined-index, concordance-gated, spike-first).
