...
| `bismark` (aligner) | `bismark-aligner` (`bismark_rs`) | 1.0.0-alpha.1 | ✅ **All 10 phases complete** (#930 = Ph 1–8, #942 = Ph 9a, #945 = Ph 9b; **Ph 10 full-scale real-data gate PASSED**, #948). Bowtie 2 backend, **SE + PE, FastQ + FastA, all 3 library types (directional / non-directional / pbat)** — read-conversion → 2–4 instances → lockstep merge/scoring/MAPQ → `XM`/`XR`/`XG` → BAM + report + `--unmapped`/`--ambiguous`/`--ambig_bam`, **byte-identical** to Perl v0.25.1 + Bowtie 2 2.5.5 at 1M reads/pairs and **content byte-identical at full real-data scale** (Ph 10 on oxy: 84M SE / 84M PE / 46.7M mouse-RRBS GRCm39 / pbat; 173/181/52 contigs; + V13 cross-check vs the pre-existing Perl `--parallel 4` BAMs). **Order-preserving `--multicore`/`--parallel`** (worker-count-invariant). The ~74% runtime "big beast" — **faithful (Bowtie 2) port complete**. **v1.x: HISAT2 backend COMPLETE** (SE+PE, FastQ+FastA, all 3 libraries — **byte-identical** to Perl v0.25.1 + HISAT2 2.2.2, oxy-gated 10k+1M; `--multicore`+`--hisat2` rejected since HISAT2's splice-site discovery is input-batch-global; v1.x Ph 2a/2b, #949). **minimap2 backend = v1.x (in progress)** — `rust/aligner-v1x`, epic `plans/06052026_bismark-aligner-v1x/` |
...
- 🚧 **Now: `bismark` aligner v1.x — HISAT2 ✅ done, minimap2 next** (epic `plans/06052026_bismark-aligner-v1x/`, branch `rust/aligner-v1x`). **HISAT2 backend byte-identical to Perl v0.25.1 + HISAT2 2.2.2** (SE+PE, all libraries; Ph 1 determinism spike → 2a SE → 2b PE, oxy-gated, #949). **minimap2 = the active phase** (Ph 3 determinism + both-strand-selection spike → Ph 4 wrapper + merge adaptation), then a combined full-scale gate (Ph 5). Then v2 (rammap / combined-index, concordance-gated, spike-first).
