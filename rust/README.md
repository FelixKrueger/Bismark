...
| `bismark` (aligner) | `bismark-aligner` (`bismark_rs`) | 1.0.0-alpha.1 | 🚧 **Phases 1–8 of 10 merged** (#930); 9–10 in progress on `rust/aligner`. Bowtie 2 backend, **SE + PE FastQ, all 3 library types (directional / non-directional / pbat)** — read-conversion → 2–4 instances → lockstep merge/scoring/MAPQ → `XM`/`XR`/`XG` → BAM + report + `--unmapped`/`--ambiguous`/`--ambig_bam`, **byte-identical** to Perl v0.25.1 + Bowtie 2 2.5.5 (oxy, all 4 mode×layout cells, 1M reads/pairs). The ~74% runtime "big beast". Remaining: FastA + order-preserving threading (Ph 9), full-scale real-data gate (Ph 10); HISAT2/minimap2 = v1.x |
