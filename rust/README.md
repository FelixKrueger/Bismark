...
| `bismark` (aligner) | `bismark-aligner` (`bismark_rs`) | 1.0.0-alpha.1 | ✅ **All 10 phases complete** (#930 = Ph 1–8, #942 = Ph 9a, #945 = Ph 9b; **Ph 10 full-scale real-data gate PASSED**, this PR). Bowtie 2 backend, **SE + PE, FastQ + FastA, all 3 library types (directional / non-directional / pbat)** — read-conversion → 2–4 instances → lockstep merge/scoring/MAPQ → `XM`/`XR`/`XG` → BAM + report + `--unmapped`/`--ambiguous`/`--ambig_bam`, **byte-identical** to Perl v0.25.1 + Bowtie 2 2.5.5 at 1M reads/pairs and **content byte-identical at full real-data scale** (Ph 10 on oxy: 84M SE / 84M PE / 46.7M mouse-RRBS GRCm39 / pbat; 173/181/52 contigs; + V13 cross-check vs the pre-existing Perl `--parallel 4` BAMs). **Order-preserving `--multicore`/`--parallel`** (worker-count-invariant). The ~74% runtime "big beast" — **faithful port complete**. HISAT2/minimap2 = v1.x |
...
- ✅ **The faithful `bismark` aligner port is COMPLETE** — all 10 phases; Phase 10 full-scale real-data gate PASSED (this PR). Byte/content-identical to Perl v0.25.1 + Bowtie 2 2.5.5 across SE+PE, FastQ+FastA, all library types, at full real-data scale (incl. mouse GRCm39 + pbat). **Next:** v1.x (HISAT2 / minimap2 aligner wrappers) + the v2 alternative-engines epic (rammap / combined-index, concordance-gated, spike-first).
