...
//!    **Rust** `crate::bedgraph::Cli` / `crate::coverage2cytosine::Cli`
...
/// `crate::bedgraph::Cli`. Every emitted flag is one the **Rust**
...
/// Build the argv for the in-process `crate::coverage2cytosine::Cli`. Every
...
///
/// Invoked from [`crate::extractor::state::ExtractState::finalize`] after the split files +
...
///
/// `sorted` is the `crate::bedgraph::aggregate::ChrPositions` slice produced by
...
pub fn run_downstream_chain(
    ...
    output_dir: &Path,
    kept_split_files: &[PathBuf],
    sorted: &[crate::bedgraph::aggregate::ChrPositions],
    is_empty_run: bool,
    ...
    if !usable && !is_empty_run {
        let logger = crate::extractor::logging::Logger::from_config(config);
        logger.note(
    ...
    let b2bg_cli = crate::bedgraph::Cli::try_parse_from(&b2bg_full).map_err(|e| {
    ...
    // `crate::bedgraph::run(&b2bg_cfg)` file-read so the `.cov.gz` is written
    ...
    crate::bedgraph::output::write_outputs_from_sorted(&b2bg_cfg, sorted).map_err(|e| {
        BismarkExtractorError::Downstream {
            tool: "bismark2bedGraph",
            message: e.to_string(),
        }
    ...
    if b2bg_cfg.ucsc {
        crate::bedgraph::ucsc::write_ucsc(&b2bg_cfg).map_err(|e| {
            BismarkExtractorError::Downstream {
                tool: "bismark2bedGraph",
                message: e.to_string(),
            }
    ...
    if config.cytosine_report {
        ...
        let c2c_cli = crate::coverage2cytosine::Cli::try_parse_from(&c2c_full).map_err(|e| {
            BismarkExtractorError::Downstream {
        ...
        crate::coverage2cytosine::run(&c2c_cfg).map_err(|e| BismarkExtractorError::Downstream {
            tool: "coverage2cytosine",
            message: e.to_string(),
        })?;
    }
...
mod tests {
    use super::*;
    use crate::extractor::cli::{OutputMode, PairedMode};
    ...
    fn argv_parses_into_each_rust_cli() {
        ...
        let mut b2bg_full: Vec<OsString> = vec!["bismark2bedGraph".into()];
        b2bg_full.extend(b2bg_argv);
        crate::bedgraph::Cli::try_parse_from(&b2bg_full)
            .expect("bedGraph argv must parse into crate::bedgraph::Cli");
        ...
        let mut c2c_full: Vec<OsString> = vec!["coverage2cytosine".into()];
        c2c_full.extend(c2c_argv);
        crate::coverage2cytosine::Cli::try_parse_from(&c2c_full)
            .expect("c2c argv must parse into crate::coverage2cytosine::Cli");
    }
    ...
    fn argv_parses_into_each_rust_cli_minimal() {
        ...
        let mut b2bg_full: Vec<OsString> = vec!["bismark2bedGraph".into()];
        b2bg_full.extend(b2bg_argv);
        crate::bedgraph::Cli::try_parse_from(&b2bg_full).expect("minimal bedGraph argv parses");
        ...
        let mut c2c_full: Vec<OsString> = vec!["coverage2cytosine".into()];
        c2c_full.extend(c2c_argv);
        crate::coverage2cytosine::Cli::try_parse_from(&c2c_full).expect("minimal c2c argv parses");
    }
}
