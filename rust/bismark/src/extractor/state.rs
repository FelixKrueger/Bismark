...
use crate::extractor::cli::{OutputMode, ResolvedConfig};
use crate::extractor::error::BismarkExtractorError;
use crate::extractor::mbias::MbiasTable;
use crate::extractor::mbias_writer::{mbias_txt_path, write_mbias_txt};
use crate::extractor::output::{OutputFileMap, SplittingReport, write_splitting_report};
...
pub struct ExtractState {
    ...
    pub bedgraph_aggregator: Option<crate::bedgraph::Aggregator>,
...
impl ExtractState {
    ...
    pub fn new(
        ...
        Ok(Self {
            ...
            bedgraph_aggregator: if config.bedgraph {
                Some(crate::bedgraph::Aggregator::new())
            } else {
                None
            },
    ...
    pub fn finalize(&mut self, config: &ResolvedConfig) -> Result<(), BismarkExtractorError> {
        ...
        let finalization = if !self.mbias_only {
            ...
            crate::extractor::output::FinalizationReport::default()
        };
        ...
        if config.bedgraph {
            ...
            let sorted = self
                ...
                .map(|a| a.into_sorted())
                .unwrap_or_default();
            crate::extractor::downstream_filenames::run_downstream_chain(
                config,
                &raw_filename,
                &config.output_dir,
                &finalization.kept,
...
/// [`crate::extractor::downstream_filenames::derive_bedgraph_filename`].
///
/// **Phase G rev 2 (code-review A L1 fix), preserved**: separated from
/// [`crate::extractor::pipeline::derive_basename`] because that function strips
