...
//! pipeline ([`crate::aligner::process_se_chunk`]/[`crate::aligner::process_pe_chunk`]) on each chunk
...
//! 3. **Commutative counter sum** — [`crate::aligner::merge::Counters::merge`] is field-wise
...
use crate::aligner::aux_out::{self, AuxKind};
use crate::aligner::config::{LibraryType, ReadFormat, RunConfig};
use crate::aligner::convert::ConvertOptions;
...
use crate::aligner::output::{build_refid, generate_sam_header};
use crate::aligner::report::{self, ReportHeader};
use crate::aligner::{PeSinks, Sinks};
...
fn count_effective(
    ...
    let mut reader = crate::aligner::open_reader(input)?;
    let mut buf = Vec::new();
    let mut count: u64 = 0;
    let mut eff: u64 = 0;
...
fn count_effective_pe(
    ...
    let mut a = crate::aligner::open_reader(r1)?;
    let mut b = crate::aligner::open_reader(r2)?;
...
fn open_chunk_se_sinks(
    ...
    let plain = |p: Option<&Path>| -> Result<Option<crate::aligner::AuxWriter>> {
        Ok(match p {
            Some(p) => Some(crate::aligner::AuxWriter::Plain(BufWriter::new(
                File::create(p)?,
            ))),
            None => None,
        })
...
/// [`crate::aligner::process_se_chunk`] against the subset, finalise, and clean up the
...
fn se_chunk_job(
    ...
    let converted = crate::aligner::process_se_chunk(
        cfg,
        genome,
        refid,
        subset,
...
fn pe_chunk_job(
    ...
    let mut counters = Counters::default();
    let converted = crate::aligner::process_pe_chunk(
        cfg,
        genome,
        refid,
        subset_1,
...
pub(crate) fn run_se_multicore(config: &RunConfig, reads: &[String], n: u32) -> Result<()> {
    ...
    for read_file in reads {
        ...
        let tok = cfg.aligner.token();
        let bam_path = crate::aligner::derive_output_path(
            read_file,
            &cfg,
            &format!("_bismark_{tok}.bam"),
            ".bam",
        );
        eprintln!(
        ...
        let filename = crate::aligner::basename(read_file);
        ...
        let report_path = crate::aligner::derive_output_path(
            read_file,
            &cfg,
        ...
        cleanup_se(&outcomes);
        eprintln!("{}", crate::aligner::counters_summary(read_file, &total));
    }
    Ok(())
}
...
pub(crate) fn run_pe_multicore(
    ...
    for (read_1, read_2) in mates1.iter().zip(mates2) {
        ...
        let (b1, b2) = (
            crate::aligner::basename(read_1),
            crate::aligner::basename(read_2),
        );
        let (prefix, base) = (cfg.output.prefix.as_deref(), cfg.output.basename.as_deref());
        let merge_pe_aux =
        ...
        eprintln!(
            "{}",
            crate::aligner::counters_summary_pe(read_1, read_2, &total)
        );
    }
    Ok(())
}
