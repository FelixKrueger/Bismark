...
use crate::io::{BamWriter, BismarkRecord};

use crate::aligner::error::{AlignerError, Result};
use crate::aligner::genome::Genome;
use crate::aligner::merge::{BestAlignment, BestAlignmentPaired};
use crate::aligner::methylation::{
    Conversion, GenomicExtraction, GenomicExtractionPaired, parse_cigar,
};
...
pub fn generate_sam_header(genome: &Genome, command_line: &str) -> Header {
    ...
    prog.other_fields_mut().insert(
        VERSION,
        BString::from(crate::aligner::BISMARK_VERSION.as_bytes()),
    );
...
mod tests {
    ...
    fn record_roundtrips_through_bam_tag_order_values_qual() {
        ...
        let mut r = crate::io::BamReader::from_path(&path).unwrap();
