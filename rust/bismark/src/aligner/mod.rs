...
use crate::aligner::report::ReportHeader;
...
pub fn version_string() -> String {
    crate::meta::version_line("bismark")
}
...
pub fn run(cli: &cli::Cli, command_line: String) -> Result<()> {
    ...
    if config.aligner == Aligner::Rammap {
        ...
        // It reads `config.rammap_subprocess` on both builds (feature-off → in-process is
        // never selected → "subprocess"), so the config field is never a feature-off dead
        // field.
        let inprocess = use_se_inprocess_rammap(&config);
        // Name the active backend. rev2 (Alt-1): the in-process pool size is the SINGLE
        // SOURCE OF TRUTH `config.rammap_inprocess_threads` — the SAME value the pool
        // builder reads (`build_se_inprocess_streams`) — so the printed count can never
        // disagree with the built pool. Auto-threaded by default; `--multicore 1` → 1.
        let backend = if inprocess {
            let n = config.rammap_inprocess_threads;
            if n > 1 {
                format!("in-process rammap-core backend: lower RAM, {n}-thread")
            } else {
                // n == 1 either because `--multicore 1` was given or because the host has
                // one core / `available_parallelism()` failed — don't misattribute it to the
                // flag; just report the honest thread count.
                "in-process rammap-core backend: lower RAM, single-threaded".to_string()
            }
        } else {
        ...
        // subprocess — i.e. specifically because the input is FastA (rev2 re-key: condition
        // is `!config.rammap_subprocess`, not the dropped `rammap_inprocess`). Feature-gated;
        // an explicit `--rammap_subprocess` run never reaches here (it asked for subprocess).
        #[cfg(feature = "rammap-inprocess")]
        if !inprocess && !config.rammap_subprocess && !matches!(config.format, ReadFormat::FastQ) {
            eprintln!(
                "Note: the in-process rammap backend supports FastQ input only; this FastA \
...
fn run_five_base_consensus_standalone(cli: &cli::Cli, command_line: &str) -> Result<()> {
    ...
    let mut probe = crate::io::BamReader::from_path_without_sort_check(&bams[0]).map_err(|e| {
    ...
    let umi_swap = if cli.five_base_umi_qname {
        Some(crate::aligner::five_base_duplex::UmiSwap::DualPlus)
    } else if cli.five_base_umi_len > 0 {
        Some(crate::aligner::five_base_duplex::UmiSwap::RevComp)
    } else {
        None
    };
...
fn process_se_chunk(
    ...
    // Phase 2 (epic 06152026; #995; rev2 Alt-1): when the in-process rammap backend is
    // compiled in and selected (SE FastQ `--rammap` by DEFAULT, unless `--rammap_subprocess`),
    // drive the merge over `Arc<rammap::Aligner>`-backed streams (each converted `.mmi` loaded
    // ONCE, shared across the 2/4 instances) instead of spawning 2/4 rammap subprocesses.
    // The streams map their reads in PARALLEL on a shared pool (sized by
    // `config.rammap_inprocess_threads`, rev2); `drive_merge` is generic over `SamStream`; the
    // merge / XM / output arm is
...
/// `config.rammap_subprocess` in always-compiled code so the field is never a feature-off
/// dead field (`-D warnings`).
///
/// **rev2 (Alt-1): `--rammap` DEFAULTS to the in-process path; the subprocess path is the
/// explicit `--rammap_subprocess` OPT-OUT.** Taken iff NOT `config.rammap_subprocess` (+
/// rammap + FastQ + feature-on). (Was Phase-4 Option A: subprocess default / in-process
/// opt-in — reversed on the post-#995 parallelization grounds; the in-process default is now
/// auto-threaded, so it is faster + leaner, not the former ~6× single-threaded regression.)
///
/// **#995: NO `multicore` gate.** The in-process path is N-threaded (a shared rayon pool
/// sized by `config.rammap_inprocess_threads`), so it runs for ANY N. A parallel FORK worker
/// reaching this branch is prevented by TWO independent mechanisms: (1) `pipeline()`
/// short-circuits the in-process path to the single-process `run_se` (reusing THIS
/// predicate), so `parallel::run_se_multicore`/`se_chunk_job` is never invoked for it; (2)
/// the `aligner == Rammap` conjunct — a fork worker only runs non-rammap engines, so even if
...
fn use_se_inprocess_rammap(config: &RunConfig) -> bool {
    inprocess_rammap_selected(config.aligner, config.rammap_subprocess, config.format)
}
...
/// the first conjunct → always `false` on the feature-off build (so `--rammap` uses the
/// subprocess there — the in-process path isn't compiled). rev2 (Alt-1): the middle conjunct
...
fn build_se_inprocess_streams(
    ...
    // ONE shared rayon pool, `Arc::clone`d into EVERY stream (NOT one pool per stream): the
    // lockstep merge drains+refills one stream at a time, so refills never overlap → a shared
    // pool of N gives the same throughput with N total threads, not instances×N. rev2 (Alt-1):
    // sized by `config.rammap_inprocess_threads` — the SINGLE SOURCE OF TRUTH also read by the
    // backend notice, so the printed count matches the built pool. Auto-threaded by default;
    // `--multicore N` verbatim; `--multicore 1` → a 1-thread pool (byte-identical to the
    // former one-at-a-time path). Output is thread-count-invariant (the parallel refill is a
    // pure per-read map + order-preserving indexed collect), so N never changes the result.
    let threads = config.rammap_inprocess_threads;
    let pool = Arc::new(
...
fn five_base_emit_record(
    ...
    baseq: u8,
    counters: &mut Counters,
) -> Result<Option<crate::io::BismarkRecord>> {
    if rec.flag & 0x4 != 0 {
    ...
    while let Some((id1, seq1, _p1, qual1)) = read_fastq_record(&mut r1)? {
        ...
        if let Some((mut out1, mut out2)) = five_base_emit_pe_record(
            ...
            if config.five_base_umi_qname {
                if let Some(rx) = crate::io::umi::extract_barcode(identifier.as_bytes()) {
                    out1.set_rx(rx);
                    out2.set_rx(rx);
                }
            } else if umi_len > 0 {
...
fn run_five_base_deconvolution(genome: &Genome, bam_path: &Path, report_path: &Path) -> Result<()> {
    use crate::aligner::five_base_deconv::{
        DEFAULT_MIN_OPP_DEPTH, DEFAULT_VARIANT_OPP_FRAC, Deconvoluter,
    };
    ...
    for rec in reader.records() {
        ...
        let xr = crate::io::tags::xr(inner.data())
            .map_err(|e| AlignerError::Validation(format!("deconvolution: XR: {e}")))?;
        let xg = crate::io::tags::xg(inner.data())
            .map_err(|e| AlignerError::Validation(format!("deconvolution: XG: {e}")))?;
        let strand = crate::io::BismarkStrand::from_xr_xg(xr, xg)
            .map_err(|e| AlignerError::Validation(format!("deconvolution: strand: {e}")))?;
        let (is_ot, is_ob) = (
            strand == crate::io::BismarkStrand::OT,
            strand == crate::io::BismarkStrand::OB,
        );
        if !is_ot && !is_ob {
...
fn run_five_base_duplex_pe(
    ...
    use crate::aligner::five_base_duplex::{DuplexFamilies, DuplexKey, SiteObs, canonical_umi};
    use noodles_sam::alignment::record::cigar::op::Kind;
    ...
    for rec in reader.records() {
        ...
        let canon_umi: Vec<u8> = if let Some(swap) = umi_swap {
            match crate::io::tags::rx(inner.data()) {
...
fn w_duplex_report<W: std::io::Write>(
    w: &mut W,
    fams: &crate::aligner::five_base_duplex::DuplexFamilies,
) -> std::io::Result<crate::aligner::five_base_duplex::DuplexSummary> {
    use crate::aligner::five_base_deconv::DEFAULT_VARIANT_OPP_FRAC;
    use crate::aligner::five_base_duplex::DUPLEX_MIN_OPP_DEPTH;
...
/// rule ([`crate::aligner::five_base_duplex::consensus_base`]): at a CpG the own strand carries
...
fn run_five_base_consensus(
    ...
    use crate::aligner::five_base_duplex::{SiteKind, canonical_umi, consensus_base};
    use noodles_sam::alignment::record::cigar::op::Kind;
    ...
    match decision {
        ...
        Decision::UniqueBest(best) => {
            ...
            if barcode_umi.enabled() {
                let (bc, umi) = crate::aligner::output::parse_barcode_umi(identifier);
...
fn drive_merge_combined<S: SamStream>(
    ...
    loop {
        ...
        let mut records: Vec<crate::aligner::align::SamRecord> = Vec::new();
...
fn drive_merge_combined_nondir<C: SamStream, G: SamStream>(
    ...
    loop {
        ...
        // re-read input.
        let mut ct_records: Vec<crate::aligner::align::SamRecord> = Vec::new();
        while ct_stream.current().is_some_and(|r| r.qname == identifier) {
            ct_records.push(ct_stream.current().unwrap().clone());
            ct_stream.advance()?;
        }
        let mut ga_records: Vec<crate::aligner::align::SamRecord> = Vec::new();
...
fn select_and_route_se_nondir(
    ct_records: &[crate::aligner::align::SamRecord],
    ga_records: &[crate::aligner::align::SamRecord],
    identifier: &str,
    sequence: &str,
    seq_uc: &[u8],
    qual_bytes: &[u8],
...
fn drive_merge_combined_pe<S: PairedSamStream>(
    ...
    loop {
        ...
        let mut pairs: Vec<crate::aligner::align::SamPair> = Vec::new();
        while stream
            .current_pair()
...
fn assert_pe_pass_in_sync(
    head: Option<&crate::aligner::align::SamPair>,
    identifier: &str,
    pass: &str,
) -> Result<()> {
    if let Some(p) = head
...
fn drive_merge_combined_pe_nondir<C: PairedSamStream, G: PairedSamStream>(
    ...
    loop {
        ...
        let mut ct_pairs: Vec<crate::aligner::align::SamPair> = Vec::new();
        while ct_stream
            .current_pair()
            .is_some_and(|p| p.seq_id == identifier)
        {
        ...
        let mut ga_pairs: Vec<crate::aligner::align::SamPair> = Vec::new();
        while ga_stream
            .current_pair()
            .is_some_and(|p| p.seq_id == identifier)
        {
...
fn select_and_route_pe_nondir(
    ct_pairs: &[crate::aligner::align::SamPair],
    ga_pairs: &[crate::aligner::align::SamPair],
    identifier: &str,
    id2_stripped: &str,
    s1: &str,
    s2: &str,
...
mod tests {
    ...
    fn five_base_emit_forward_methylated_cpg() {
        ...
        let xm = crate::io::tags::xm(inner.data()).unwrap();
        assert_eq!(
            xm, b"Z...",
    ...
    fn five_base_emit_forward_unmethylated_cpg() {
        ...
        .unwrap()
        .unwrap();
        let xm = crate::io::tags::xm(out.inner().data()).unwrap();
        assert_eq!(xm, b"z...");
    ...
    fn five_base_emit_pe_forward_pair_methylated_and_flags() {
        ...
        let xm1 = crate::io::tags::xm(o1.inner().data()).unwrap();
        assert_eq!(
            xm1, b"Z...",
    ...
    fn inprocess_rammap_predicate_truth_table() {
        ...
        // rev2 (Alt-1): rammap + NOT opted-out + FastQ selects in-process IFF the feature is on.
        let selected = cfg!(feature = "rammap-inprocess");
        assert_eq!(
            inprocess_rammap_selected(Aligner::Rammap, false, ReadFormat::FastQ),
            selected
        );
        // Opted OUT (`--rammap_subprocess`) → subprocess, on both builds.
        assert!(!inprocess_rammap_selected(
            Aligner::Rammap,
            true,
            ReadFormat::FastQ
        ));
        // Wrong aligner → subprocess even by default (no opt-out), on both builds.
        assert!(!inprocess_rammap_selected(
            Aligner::Bowtie2,
            false,
            ReadFormat::FastQ
        ...
        // FastA → subprocess even by default (the in-process stream is 4-line FastQ-only).
        assert!(!inprocess_rammap_selected(
            Aligner::Rammap,
            false,
            ReadFormat::FastA
    ...
    struct VecStream {
        recs: Vec<crate::aligner::align::SamRecord>,
        idx: usize,
    }
    impl crate::aligner::align::SamStream for VecStream {
        fn current(&self) -> Option<&crate::aligner::align::SamRecord> {
    ...
    fn spill_stream_to_file_round_trips_incl_unmapped() {
        ...
        let recs: Vec<_> = lines
            .iter()
            .map(|l| crate::aligner::align::SamRecord::parse(l).unwrap())
            .collect();
        ...
        let mut replay = crate::aligner::align::FileSamStream::open(&p).unwrap();
        for orig in &recs {
    ...
    fn spill_stream_to_file_empty_stream() {
        ...
        assert!(
            crate::aligner::align::FileSamStream::open(&p)
                .unwrap()
                .current()
                .is_none()
        );
    ...
    struct VecPairStream {
        pairs: Vec<crate::aligner::align::SamPair>,
        idx: usize,
    }
    impl PairedSamStream for VecPairStream {
        fn current_pair(&self) -> Option<&crate::aligner::align::SamPair> {
    ...
    fn spill_pe_stream_to_file_round_trips_incl_miss() {
        let dir = tempfile::TempDir::new().unwrap();
        let p = dir.path().join("ct_pass.sam");
        let mk = |l1: &str, l2: &str| crate::aligner::align::SamPair::from_lines(l1, l2).unwrap();
        let pairs = vec![
            mk(
    ...
    fn assert_pe_pass_in_sync_fires_on_mismatch() {
        let pair = crate::aligner::align::SamPair::from_lines(
    ...
    fn assert_pe_tag_in_sync_fires_on_mismatch_and_untagged() {
        ...
        let tagged = crate::aligner::align::SamPair::from_lines(
        ...
        let untagged = crate::aligner::align::SamPair::from_lines(
            "rX/1\t99\tchr1_CT_converted\t1\t40\t6M\t=\t1\t6\tACGTAC\tFFFFFF\tAS:i:0\tMD:Z:6",
            "rX/2\t147\tchr1_CT_converted\t1\t40\t6M\t=\t1\t-6\tACGTAC\tFFFFFF\tAS:i:0\tMD:Z:6",
        )
        .unwrap();
    ...
    fn modelb_pe_tag_before_suffix_round_trips() {
        ...
        let ga = crate::aligner::align::SamPair::from_lines(
        ...
        let broken = crate::aligner::align::SamPair::from_lines(
