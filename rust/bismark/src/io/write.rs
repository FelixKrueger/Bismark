...
//!
//! Mirrors the structure of [`crate::io::read`]:
//!
...
use crate::io::cram_ref::build_fasta_repository;
use crate::io::error::BismarkIoError;
use crate::io::read::AlignmentKind;
...
/// (see [`crate::io::cram_ref::build_fasta_repository`]). Auto-fai-generation
/// is future work.
///
...
/// Path-dispatching writer enum. Mirrors [`crate::io::read::AnyReader`] for
...
mod tests {
    ...
    use crate::io::read::{BamReader, SamReader};
    use bstr::BString;
    use noodles_sam::alignment::RecordBuf;
    ...
    fn cram_writer_roundtrip_via_tempfile() {
        ...
        let mut reader = crate::io::read::CramReader::from_path(&cram_path, &fasta_path).unwrap();
        let records: Vec<_> = reader.records().collect();
    ...
    fn threaded_bam_writer_finish_writes_bgzf_eof_marker() {
        ...
        {
            let mut writer = crate::io::write::ThreadedBamWriter::from_path(
                &bam_path,
                synth_header(),
    ...
    fn threaded_bam_writer_roundtrip_via_tempfile() {
        ...
        let mut reader = crate::io::read::ThreadedBamReader::from_path(
            &bam_path,
            std::num::NonZero::new(4).unwrap(),
        )
        .unwrap();
        ...
        for r in records {
            let rec = r.unwrap();
            assert_eq!(rec.record_strand(), crate::io::strand::BismarkStrand::OT);
        }
    }
    ...
    fn threaded_bam_writer_output_readable_by_single_threaded_reader() {
        ...
        let mut reader = crate::io::read::BamReader::from_path(&bam_path).unwrap();
