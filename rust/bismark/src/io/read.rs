...
impl AlignmentKind {
    ...
    ///
    /// Used by [`crate::io::open_writer`] (where the file doesn't exist yet,
    ...
    ///
    /// Used by [`crate::io::open_reader`] and any caller that wants
...
impl<R: BufRead> BamReader<R> {
    ...
    ///
    /// `extractor` is typically [`crate::io::umi::extract_barcode`] (for
    /// `--barcode` / `--umi` mode) or [`crate::io::umi::extract_bclconvert`]
...
// in `crate::io::cram_ref` so both reader and writer can share it.
...
read1\t0\tchr1\t10\t60\t5M\t*\t0\t0\tACGTC\tIIIII\tXM:Z:.....\tXG:Z:CT\n";
    ...
    fn sam_reader_yields_mapped_record() {
        ...
        assert_eq!(rec.record_strand(), crate::io::BismarkStrand::OT);
    }
...
read1:CTCCTTAG\t0\tchr1\t10\t60\t5M\t*\t0\t0\tACGTC\tIIIII\tXM:Z:.....\tXR:Z:CT\tXG:Z:CT\n";
    ...
    fn sam_records_with_umi_populates_umi_field() {
        ...
        let records: Vec<_> = reader
            .records_with_umi(crate::io::umi::extract_barcode)
            .collect();
        assert_eq!(records.len(), 1);
        let rec = records.into_iter().next().unwrap().unwrap();
