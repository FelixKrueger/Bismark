...
pub enum BismarkIoError {
    ...
    /// [`crate::io::AlignmentKind::from_extension`] (writer-side dispatch — the
    /// output file doesn't exist yet, so content sniffing is impossible).
    /// Reader-side dispatch via [`crate::io::AlignmentKind::from_path`] sniffs
    ...
    /// format detection.
    ///
    /// Emitted by [`crate::io::AlignmentKind::from_path`] when the file is
    ...
    /// header), or `C` (CRAM).
    ///
    /// Emitted by [`crate::io::AlignmentKind::from_path`]. Common causes:
    ...
    ///
    /// Emitted by [`crate::io::AlignmentKind::from_path`].
    #[error(
