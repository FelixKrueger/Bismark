...
pub mod cli;
#[allow(clippy::module_inception)] // core logic module; named `dedup` as a crate, kept on fold
pub mod dedup;
pub mod error;
pub mod filename;
pub mod pipeline;
...
use crate::dedup::cli::Cli;
use crate::dedup::cli::ResolvedConfig;
...
pub fn version_string() -> String {
    crate::meta::version_line("deduplicate_bismark")
}
...
fn run(cli: Cli) -> Result<(), BismarkDedupError> {
    ...
    if config.umi_mode == Some(crate::dedup::cli::UmiMode::Barcode) {
    ...
    match config.umi_mode {
        Some(crate::dedup::cli::UmiMode::Barcode) => {
            eprintln!("Deduplicating data in UMI mode");
        }
        Some(crate::dedup::cli::UmiMode::Bclconvert) => {
...
fn process_one(input: &Path, config: &ResolvedConfig) -> Result<(), BismarkDedupError> {
    ...
    let kind = crate::io::AlignmentKind::from_path(input)?;
    let use_parallel = config.parallel > 1 && matches!(kind, crate::io::AlignmentKind::Bam);

    if config.parallel > 1 && matches!(kind, crate::io::AlignmentKind::Cram) {
        eprintln!(
...
fn process_multiple(config: &ResolvedConfig) -> Result<(), BismarkDedupError> {
    ...
    let kinds: Result<Vec<_>, _> = config
        .files
        .iter()
        .map(|p| crate::io::AlignmentKind::from_path(p))
        .collect();
    let kinds = kinds?;
    let all_bam = kinds.iter().all(|k| *k == crate::io::AlignmentKind::Bam);
    let any_cram = kinds.contains(&crate::io::AlignmentKind::Cram);
...
fn check_bclconvert_format_conflict(
    ...
    let mut reader = crate::io::open_reader(input, config.cram_ref.as_deref())?;
    ...
    if crate::io::umi::extract_bclconvert(&first_qname).is_some() {
...
fn resolve_paired_mode(input: &Path, config: &ResolvedConfig) -> Result<bool, BismarkDedupError> {
    ...
    let reader = crate::io::open_reader(input, config.cram_ref.as_deref())?;
