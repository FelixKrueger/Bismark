...
//! [`crate::dedup::dedup::DedupState`] primitives into a working dedup loop.
//!
...
use crate::io::AnyWriter;
use crate::io::BismarkPair;
...
pub use crate::io::detect_paired_from_header;
use bstr::BString;
...
use crate::dedup::UmiDedupKey;
use crate::dedup::UmiDedupState;
use crate::dedup::cli::UmiMode;
...
// v1.0.0-beta.7. Use `crate::io::detect_paired_from_header` directly
...
/// [`crate::io::ThreadedBamWriter`]. Lets `stream_se` / `stream_pe`
/// be generic over the writer type — same dedup logic, different
/// underlying BGZF concurrency.
trait WriteBismark {
    fn write_one(&mut self, record: &BismarkRecord) -> Result<(), crate::io::BismarkIoError>;
}
...
impl WriteBismark for crate::io::ThreadedBamWriter {
    fn write_one(&mut self, record: &BismarkRecord) -> Result<(), crate::io::BismarkIoError> {
        crate::io::ThreadedBamWriter::write_record(self, record)
    }
}

/// Stream SE records: per-record, compute key, observe, write on unique.
fn stream_se<W: WriteBismark>(
    records: impl Iterator<Item = Result<BismarkRecord, crate::io::BismarkIoError>>,
    refid_table: &[u32],
...
pub fn run_single(
    ...
    let mut writer = crate::io::open_writer(output, header, cram_ref)?;
...
pub fn run_multiple(
    ...
    let first_kind = crate::io::AlignmentKind::from_path(&inputs[0])?;
    for path in &inputs[1..] {
        if crate::io::AlignmentKind::from_path(path)? != first_kind {
    ...
    let readers: Vec<_> = inputs
        .iter()
        .map(|p| crate::io::open_reader(p, cram_ref))
...
pub fn run_single_parallel(
    ...
    let mut writer = crate::io::ThreadedBamWriter::from_path(output, header, parallel)?;
    let mut state = DedupState::new();
...
pub fn run_multiple_parallel(
    ...
    for path in inputs {
        if crate::io::AlignmentKind::from_path(path)? != crate::io::AlignmentKind::Bam {
            return Err(BismarkDedupError::MultipleMixedFormat);
        }
    }

    // Open all readers (threaded) and capture their headers up front.
    let mut readers: Vec<crate::io::ThreadedBamReader> = inputs
        .iter()
        .map(|p| crate::io::ThreadedBamReader::from_path(p, parallel))
        .collect::<Result<Vec<_>, _>>()?;
    let headers: Vec<Header> = readers.iter().map(|r| r.header().clone()).collect();
...
/// Map a [`UmiMode`] to the corresponding [`crate::io::umi`] extractor
...
fn umi_extractor_for(mode: UmiMode) -> fn(&[u8]) -> Option<&[u8]> {
    match mode {
        UmiMode::Barcode => crate::io::extract_barcode,
        UmiMode::Bclconvert => crate::io::extract_bclconvert,
    }
}
