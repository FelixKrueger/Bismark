...
use bismark::io::{BamReader, BismarkPair, BismarkStrand, ReadIdentity, ThreadedBamReader};
...
fn threaded_bam_reader_preserves_record_order() {
    fn qnames_of(
        reader: impl Iterator<Item = Result<bismark::io::BismarkRecord, bismark::io::BismarkIoError>>,
    ) -> Vec<String> {
        reader
            .map(|r| {
                let rec = r.unwrap();
