...
fn run_oracle(ds: &AlignerDataset) {
    ...
    let mut g_reader = bismark::io::open_reader(&golden_bam, None).expect("open golden BAM");
    let mut c_reader = bismark::io::open_reader(&cur_bam, None).expect("open current BAM");
