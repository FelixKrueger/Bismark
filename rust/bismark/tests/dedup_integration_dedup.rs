...
fn read_qnames(path: &Path) -> Vec<String> {
    let mut reader = bismark::io::open_reader(path, None).unwrap();
...
fn nondir_pe_repro_1030_dedups_without_crash() {
    let fixture =
        PathBuf::from(env!("CARGO_MANIFEST_DIR")).join("tests/data/dedup/nondir_pe_1030.bam");
...
fn assert_header_only_output(out_bam: &Path) {
    ...
    let reader = bismark::io::open_reader(out_bam, None).unwrap();
    assert!(
...
fn cram_with_parallel_n_logs_warning_and_runs_single_threaded() {
    ...
    let cram_in = dir.path().join("input.cram");
    {
        ...
        let mut writer = bismark::io::open_writer(&cram_in, header, Some(&fasta)).unwrap();
        let mut records = Vec::new();
        for i in 0..5 {
...
/// Stems of the two 10K fixtures (matching the file prefixes in `tests/data/dedup/`).
...
fn perl_dedup_script() -> Option<PathBuf> {
    let script =
        PathBuf::from(env!("CARGO_MANIFEST_DIR")).join("../../legacy_perl/deduplicate_bismark");
    if !script.exists() {
        return None;
    }
