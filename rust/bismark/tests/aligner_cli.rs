...
///
/// Reads RAW `RecordBuf`s via `noodles_bam` (NOT `bismark::io::BamReader`, which would
...
fn minimap2_se_mapped_names_and_report() {
    ...
    let mut reader =
        bismark::io::BamReader::from_path(&outdir.path().join("reads_bismark_mm2.bam")).unwrap();
...
fn five_base_pe_end_to_end_inverts_polarity() {
    ...
    let mut reader = bismark::io::BamReader::from_path(&bam_path).unwrap();
    let recs: Vec<_> = reader.records().map(|r| r.unwrap()).collect();
    ...
    let xm = bismark::io::tags::xm(r1_rec.inner().data()).unwrap();
    assert_eq!(
        xm, b".Z...z",
