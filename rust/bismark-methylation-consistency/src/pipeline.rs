...
use bismark_io::{BamReader, BamWriter, BismarkIoError, BismarkRecord};
...
use crate::classify::{Bucket, Routing, classify, count_xm};
use crate::cli::{LibraryMode, ResolvedConfig};
...
fn resolve_mode(mode: LibraryMode, header: &Header, logger: &Logger) -> bool {
    match mode {
        ...
        LibraryMode::Auto => {
            let is_paired = bismark_io::detect_paired_from_header(header).unwrap_or(false);
            logger.info(&format!(
