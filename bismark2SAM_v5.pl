...
sub verify_input_file{
  ...
  if($is_paired_end){
    ...
    die "Input file ($bismark_infile) is not a single-end Bismark mapping result file\n" if scalar @NR != 11;
  }
  close $ifh or die $!;
}
...
sub convert_se_bismark_to_SAM{
  ...
  while(my $line = <$ifh>){
    ...
    ++$count;
    chomp $line;
    my ($id,$strand,$chr,$start,$stop,$actual_seq,$ref_seq,$read_conversion,$genome_conversion,$qual) = (split ("\t", $line))[0,1,2,3,4,5,6,8,9,10];
    ...
    ### this is the defaultline-by-line SAM output
      ...
      print OUT join("\t", ($id, $flag, $chr, $start, $mapq, $cigar, $mrnm, $mpos, $isize, $actual_seq, $qual, $NM_tag, $MD_tag)), "\n";
    }
  }

  if ($sorting){
    foreach my $seq (sort {$seqs{$a}->{chr} cmp $seqs{$b}->{chr} || $seqs{$a}->{start}<=>$seqs{$b}->{start} } keys %seqs){
      print OUT join("\t", ($seqs{$seq}->{id}, $seqs{$seq}->{flag}, $seqs{$seq}->{chr}, $seqs{$seq}->{start}, $seqs{$seq}->{mapq}, $seqs{$seq}->{cigar},$seqs{$seq}->{mrnm}, $seqs{$seq}->{mpos}, $seqs{$seq}->{isize}, $seqs{$seq}->{actual_seq}, $seqs{$seq}->{qual}, $seqs{$seq}->{NM_tag}, $seqs{$seq}->{MD_tag})), "\n";
    }
  }
}
...
sub convert_pe_bismark_to_SAM{
  ...
  while(my $line = <$ifh>){
    ...
    #  ($strand eq "+") ? ($flag1 = 99) : ($flag1 = 83);             #  99 if read 1 is positive strand, 83 if read 1 is negative strand  ### OLD VERSION
    ...
    $actual_seq1 = revcomp($actual_seq1) if $strand eq "-";       # Sequence represented on the forward genomic strand
    $ref_seq1 = revcomp($ref_seq1) if $strand eq "-";             # Required for comparison with actual sequence
    ...
    $actual_seq2 = revcomp($actual_seq2) if $strand eq "+";       # Mate sequence represented on the forward genomic strand
    $ref_seq2 = revcomp($ref_seq2) if $strand eq "+";             # Required for comparison with actual sequence
	...
	;
      ...
      print OUT join("\t", ($id, $flag1, $chr, $read1start, $mapq, $cigar1, $mrnm, $mpos1, $isize1, $actual_seq1, $qual1, $NM_tag1, $MD_tag1)), "\n";
      print OUT join("\t", ($id, $flag2, $chr, $read2start, $mapq, $cigar2, $mrnm, $mpos2, $isize2, $actual_seq2, $qual2, $NM_tag2, $MD_tag2)), "\n";
    }
  }
  ...
  if ($sorting){
    foreach my $seq (sort {$seqs1{$a}->{chr} cmp $seqs1{$b}->{chr} || $seqs1{$a}->{start}<=>$seqs1{$b}->{start}} keys %seqs1){
      ...
      ##seq1
      print OUT join("\t", ($seqs1{$seq}->{id}, $seqs1{$seq}->{flag}, $seqs1{$seq}->{chr}, $seqs1{$seq}->{start}, $seqs1{$seq}->{mapq}, $seqs1{$seq}->{cigar},$seqs1{$seq}->{mrnm}, $seqs1{$seq}->{mpos}, $seqs1{$seq}->{isize}, $seqs1{$seq}->{actual_seq}, $seqs1{$seq}->{qual}, $seqs1{$seq}->{NM_tag}, $seqs1{$seq}->{MD_tag})), "\n";

      ##seq2
      print OUT join("\t", ($seqs2{$seq}->{id}, $seqs2{$seq}->{flag}, $seqs2{$seq}->{chr}, $seqs2{$seq}->{start}, $seqs2{$seq}->{mapq}, $seqs2{$seq}->{cigar},$seqs2{$seq}->{mrnm}, $seqs2{$seq}->{mpos}, $seqs2{$seq}->{isize}, $seqs2{$seq}->{actual_seq}, $seqs2{$seq}->{qual}, $seqs2{$seq}->{NM_tag}, $seqs2{$seq}->{MD_tag})), "\n";
    }
  }
}
