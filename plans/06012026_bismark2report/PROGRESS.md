...
| Implementation | 🚧 Implementing | Phases A–F: **54 tests green**, clippy/fmt clean, real-data gate PASSED (10M + full-55.7M PE byte-identical). **Committed** (86714c1) + **freshened off iron-chancellor** (merge 790768e); pushed `rust/bismark2report`; **PR #931** open (MERGEABLE, base iron-chancellor); F2 status-table done; board → In Progress. Awaiting CI + "merge for me" |
...
- 2026-06-01: Committed (86714c1) + merged iron-chancellor (790768e, conflicts in Cargo.toml/lock/README resolved by union) + pushed `rust/bismark2report` → **PR #931** (base iron-chancellor, MERGEABLE); added to "Bismark Rust rewrite" board, status In Progress
...
- 2026-06-01: Literal oxy gate → ✅ PASSED — built binary on oxy (Linux); real SE 10M + dedup/splitting/M-bias + **nucleotide_stats** (bam2nuc), byte-identical (3,156,393 B); closes the SE + --nucleotide_coverage paths (PLAN §16.1)
