...
| 16 | Full-scale + asymmetric-consumer validation | ✅ Done | Mouse RRBS `SRR24766921` 10M pairs on GRCm39, 32-core box, **51 runs, one at a time**. **C1** 24 arm-pairs byte-identical, 0 differing (`--multicore 2` gives the same digest as single-chunk). **C2** 0 KB streamed vs 3.19 GiB (directional) / 6.38 GiB (non-directional) held for the whole run. **C3** never slower — −0.01 % to −1.50 % across every shape at 2M and 10M. **Asymmetry (D-6) closed**: files decouple the instances and idle at 400 % for the last 1194 s; streaming paces them to a flat 645 % and finishes together, for identical total CPU and 8.9 s less wall. `CHANNEL_DEPTH` untouched. **§7 all pass**, incl. the previously-undemonstrated premise — on a 512 MiB `--temp_dir` the file arm dies `ENOSPC` and the streamed arm completes. One 2.4 % non-directional regression did **not** reproduce and is retracted. `FULLSCALE.md` §4's `-p 1` shape is not runnable (bismark requires `-p ≥ 2`); ran `-p 2`. See [`fullscale/RESULTS.md`](./fullscale/RESULTS.md) and `BENCHMARKS.md` §3 |
...
- **D-14** (2026-09-11, user) — **cap the FIFO name instead of building a fallback.** The review flagged that a long sample name could pass the start-up probe as a file and then fail `mkfifo`. The first answer was an automatic per-chunk fallback to files, which needed `process_*_chunk` restructured. The user pointed out the simpler one: the pipes are intermediates with exactly one consumer, nothing functional reads their names, so cap the name and the failure mode disappears. `NAME_STEM_CAP = 64`, truncated on a char boundary.
- **D-15** (2026-09-11, user) — **FIFO path uniqueness must NOT come from the name.** Capping the name introduced a silent collision: `--multicore N` runs chunks concurrently in one process, so pid and slot repeat, and the only distinguishing part of the name is `.temp.<i>` at the END of the basename — exactly what the cap truncates. Two chunks would agree on a path, and since `mkfifo` unlinks first, the second would delete the first's live pipe and both would write into one. A process-wide atomic set counter now makes the path unique by construction. Caught by the user asking "this won't accidentally create duplicate filenames, right?"; the A/B matrix could not have caught it, because its fixture names are short.
- **D-17** (2026-09-12, validation) — **the asymmetric-consumer question is answered and needed no special experiment.** D-6 framed it as a benchmark to design; `FULLSCALE.md` corrected that to the natural rate difference between strand hypotheses, which happens on every run. Measured at full scale: the coupling is real — the file arm's instances diverge by 20 minutes and the machine idles at half utilisation for the last 40 % of the run, while the streamed arm holds flat and both finish together — and it is free, because wall time is set by the slowest instance either way. Total CPU differs by 3 seconds in 19,000. `CHANNEL_DEPTH` stays at 4.

- **D-16** (2026-09-11, user) — **the in-process rammap backend takes its converted reads in memory**, with no file and no FIFO. It aligns inside this process, so a temp file meant serialising records to disk for the same process to parse straight back. `InProcessAlignerStream` was already generic over `BufRead`, so `inprocess.rs` needed no change: a `ChannelReader` fed by the shared converter fan-out replaces the file. The byte-level channel is kept deliberately over passing parsed records, so the conversion loop stays unforked and byte-identity holds by construction. The SUBPROCESS rammap backend keeps files (FIFO support unverified).
