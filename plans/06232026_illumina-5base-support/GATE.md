...
- **Paired-end duplex** (`--five_base_duplex`/`--five_base_consensus` with `-1/-2`): currently rejected (SE only). This is the piece that makes the real PE data reconcile. **Design (de-risked):** key each FAMILY by `(chrom, fragment-outer-span, canonical dual UMI)` — the fragment span comes per-record from POS + mate-pos + TLEN, so the top-strand pair and the swapped-UMI bottom-strand pair of one molecule land in the same family without explicit R1/R2 pairing. **Two distinct strand concepts** (they coincide in SE, differ in PE): (1) *molecule-strand* = which duplex pair (R1 forward = OT pair / R1 reverse = OB pair; from the pair index, NOT R2's CTOT/CTOB tag) → used for family pairing (`is_paired` = both present); (2) *coverage-strand* = which genomic strand a read reads (forward FLAG → `+`, reverse → `-`) → used for per-CpG own/opposite reconciliation. R2 reads the opposite coverage-strand from its R1, so the existing OT/OB-only filter would wrongly drop R2 — the PE pass must assign own/opposite by FLAG orientation. PE consensus collapse (merging two pairs) is a further step after the PE report.
