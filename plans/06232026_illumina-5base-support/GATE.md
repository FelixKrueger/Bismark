...
- **Remaining:** (1) **PE consensus collapse** (`--five_base_consensus` PE — merge a duplex family's two read-pairs into one consensus pair/read; SE-only today); (2) a **full-depth real-data run** to actually observe duplex-paired families (needs the complete BaseSpace download — currently `.part`/incomplete — and full-genome alignment, not chr20-only).
