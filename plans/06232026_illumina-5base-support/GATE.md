...
- **Remaining:** a **full-depth real-data run** to actually observe duplex-paired families (needs the complete BaseSpace download — currently `.part`/incomplete, no lane has both R1+R2 complete — and full-genome alignment, not chr20-only). All the machinery (SE+PE align, deconvolution, duplex report, consensus collapse, inline + qname dual UMIs) is implemented and synthetic-gated.
