...
**Status:** rev 1 (2026-05-31) — dual plan-review folded (A APPROVE-WITH-CHANGES, B APPROVE; both re-ran live Perl v0.25.1 and confirmed **every** surprising claim — **zero Critical, zero byte-identity divergences**). rev-1 folds: non-contiguous-chr re-appearance handling (forbid writer/buffer caching, B-I1); NOMe×split filename + 4 added goldens (A-M2/B-I2/B-M4); threshold-precision wording (B-M2); NOMe mutex-check order (A-M1); `config`-immutability (B-M3); summary-under-threshold correction (A-nit). Ready for the implement trigger.
...
2. Add the **NOMe block** (Perl `:2147-2161`) **in Perl's order** (rev 1, A-M1): after the `--merge_CpGs`+`--CX` and `--merge_CpGs`+`--split_by_chromosome` mutex checks but **before** the `--merge_CpGs`+`--coverage_threshold` mutex check and the threshold-default block (Perl's NOMe block `:2147` precedes the threshold block `:2174`). This makes `--nome-seq --merge_CpGs --coverage_threshold 5` die with `NomeWithMerge` (not `MergeCpgsWithThreshold`), matching Perl. The NOMe block:
...
4. **`--gc` does NOT bump the threshold at CLI time.** Perl bumps `$threshold` to 1 **inside** `generate_GC_context_report` (`:758-761`), *after* the core report already ran at the original threshold. So the **core report runs at the user's `--coverage_threshold`** (default `0` when unset — `--gc` alone leaves the full uncovered genome; **`--gc --coverage_threshold N` runs the core at N too**, rev 1 B-M2) and **only the GpC walk uses an effective `max(config.threshold, 1)`**. ⚠️ Reproduce by computing a **local** `gpc_threshold = if config.threshold == 0 { 1 } else { config.threshold }` inside the GpC walk — **never mutate `ResolvedConfig.threshold`** (rev 1 B-M3; the core report + summary must see the unmodified value). (With `--nome-seq`, `config.threshold` is already ≥1 from step 2, so `gpc_threshold == config.threshold`.)
...
3. **Filenames** (Perl `handle_filehandles:119-127`): with `nome` (non-split), the core report is `{stem}.NOMe.CpG_report.txt[.gz]` and its cov companion `{stem}.NOMe.CpG.cov[.gz]`. **In `--split_by_chromosome` mode** (rev 1 A-M2) the NOMe core report/cov follow the core report's split derivation — the **raw `-o`** + `.chr<name>` infix + `.NOMe` → `{raw}.chr<name>.NOMe.CpG_report.txt[.gz]` / `…NOMe.CpG.cov[.gz]` (reuse `report_name`'s base + `.chr`-infix selection, then insert `.NOMe` before the `.CpG_report.txt`/`.CpG.cov` suffix); pinned by V19. The **context-summary** name is the **non-NOMe** base `{stem}.cytosine_context_summary.txt` (the `.NOMe` is appended to `cytosine_report_file`/`cytosine_coverage_file` *after* the summary name was taken from `cytosine_report_file` at `:115-116`). NOMe is incompatible with `--CX`, so only the `.CpG_*` (never `.CX_*`) branch applies.
...
**Per-chromosome processing** (same streaming model as the core report): re-open the coverage file (`cov::open_cov(&config.cov_infile)`), buffer cov lines per-chromosome into `pos → (meth, nonmeth)` (`cov::parse_cov_line`), flush each chromosome on `chr`-transition by walking `Genome::get(chr)`. ⚠️ **Non-contiguous chromosome re-appearance (rev 1 B-I1):** the GpC walk follows the **same contract as the core report** — a chromosome re-appearing non-adjacently in the cov is **flushed again** (single-file: re-emits its GpC block; split: **re-truncates** its per-chr file, keeping only the last segment — Perl reopens with truncation). **Do NOT cache a per-name buffer or writer** (a "process each chromosome once" optimization silently diverges from Perl on a non-contiguous cov). Reuse the core report's `run_single`/`flush_split_chromosome` pattern, which already implements this; pinned by V18. **Covered chromosomes only** — the GpC report has **no uncovered-chromosome pass** (Perl `generate_GC_context_report` only processes chromosomes seen in the cov file; `$processed{$last_chr}=1` is set but no `sort keys %processed` loop exists in this function). A cov chromosome absent from the genome yields no bytes (empty `while` walk).
...
5. The context summary is written **once** by the core report, with the **non-NOMe** base name. ⚠️ Its **content is NOT unchanged under NOMe** (rev 1, A-nit): `context_reporting` runs *after* the threshold guard (v1.0 SPEC §7.2: guard 4 < guard 6), so NOMe's `threshold = 1` gates summary accumulation to **covered positions only** — the NOMe summary differs from a plain (threshold-0) summary. No extra code is needed (the existing guard ordering produces this), but V13 must diff against **Perl's NOMe summary golden**, not a plain-run summary. (The NOMe ACG/TCG filter, applied *after* `context_reporting`, does NOT affect the summary.)
...
| V13 | NOMe summary filename + content | any `--nome-seq` run; diff vs **Perl's NOMe summary golden** | `{stem}.cytosine_context_summary.txt` (NO `.NOMe` infix); content reflects the **threshold-1-gated** core walk (covered positions only — **NOT** equal to a plain threshold-0 summary; rev 1 A-nit) |
...
| V18 | **non-contiguous chr re-appearance** (rev 1 B-I1) | a cov where chr1 appears, then chr2, then chr1 again; `--gc` (single) and `--gc --split` | byte-identical to Perl: single-file re-emits the chr1 GpC block; split re-truncates `…chrchr1.GpC.*` to the **last** chr1 segment — proves no per-name caching |
| V19 | **`--nome-seq --split_by_chromosome`** (rev 1 A-M2/B-I2) | 2-chr fixture, `--nome-seq --split` | per-chr `{raw}.chr<N>.NOMe.CpG_report.txt`/`.NOMe.CpG.cov` + `{raw}.chr<N>.NOMe.GpC_report.txt`/`.NOMe.GpC.cov`, byte-identical (the only path through Perl's `$nome`-true `close GCCOV` branch `:951-954`) |
| V20 | **`--gc --zero_based` discriminator** (rev 1 B-I2) | `--gc --zero_based`; diff BOTH the core report and the GpC report vs `--gc` | the **core** report's coords **shift** (`pos-1`), while the **GpC** report/cov stay **frozen** 1-based (no `$zero` branch) — pins the asymmetry, not just GpC-invariance |
| V21 | **`--nome-seq --zero_based` cov coord** (rev 1 B-M4) | `--nome-seq --zero_based`; the NOMe core `.cov` | the core NOMe `.cov` uses a **point** coord `(pos-1, pos-1)` (NOT merge-style half-open `pos, pos+1`); GpC `.cov` still frozen 1-based; vs Perl golden |
...
## Revision history
- **rev 0** (2026-05-31): initial Phase 1 plan from the v1.x EPIC + v1.0 SPEC rev 3 + Perl `generate_GC_context_report:751-1073` / core-report NOMe hooks / `process_commandline:2147-2161` + **live-Perl-v0.25.1 fixture confirmation** of every GpC/NOMe filename, format, ordering, and edge guard.
- **rev 1** (2026-05-31): dual plan-review folded (`PLAN_REVIEW_A.md` APPROVE-WITH-CHANGES, `PLAN_REVIEW_B.md` APPROVE; both independently re-ran live Perl v0.25.1 and confirmed **all** surprising claims — zero Critical, zero byte-identity divergences). Folded: **B-I1** non-contiguous-chr re-appearance → re-emit/re-truncate, forbid per-name writer/buffer caching (§3.3, §3.5, V18); **A-M2/B-I2** NOMe×split filename `{raw}.chr<N>.NOMe.CpG_report.txt` (§3.2.3) + goldens V18 (non-contiguous), V19 (NOMe×split), V20 (`--gc --zero_based` core-shifts/GpC-frozen discriminator), V21 (`--nome-seq --zero_based` point-coord); **B-M2** threshold-precision (`--gc --coverage_threshold N` runs core@N; §3.1 step 4); **A-M1** NOMe mutex-check order (NomeWithMerge before MergeCpgsWithThreshold; §3.1 step 2); **B-M3** never mutate `config.threshold` — local `gpc_threshold` (§3.1 step 4); **A-nit** the NOMe summary is threshold-1-gated (covered only), not equal to a plain summary (Assumption 5, V13). Remaining nits (GCGC rationale verbosity B-M1; covered-vs-last-chr reorder note) are cosmetic; the implementation guidance is unaffected. Ready for the implement trigger.

## Implementation notes (rev 2 — implemented 2026-05-31)

**Status: IMPLEMENTED.** Steps 1–7 complete; 131 crate tests green (80 lib incl. 6 GpC + 2 NOMe-kernel unit tests; 18 Phase-1 goldens; 11 B + 7 C + 10 D + 5 sanity — **no regression**); `cargo fmt --check` + `clippy --all-targets -D warnings` clean. **Not yet committed; not yet dual-code-reviewed/plan-managed** (awaiting Felix's go-ahead).

**What was built (per §5):**
- **Step 1 (CLI):** already done in commit `7dae2a7` (un-reject `--gc`/`--nome-seq`, NOMe mutexes in Perl order, threshold default). Verified intact + V1/V2/V3 unit tests present.
- **Step 5 (`report.rs` NOMe core):** `emit_position` gained `nome: bool` + `cov_out: &mut Vec<u8>` (ACG/TCG upstream filter after `context_reporting`; the `.NOMe.CpG.cov` companion = point coord honouring `--zero_based`). `chromosome_report_bytes` now returns `(report, cov)`. `run_single`/`run_split`/`flush_split_chromosome` open a `.cov` writer only under `nome`; the uncovered pass is gated `threshold == 0 && !nome`. `report_name` gained a `nome` flag (`.NOMe` infix); new `nome_cov_path`. **`pct6` promoted** from `merge.rs` → `report.rs` (`pub(crate)`); `merge::round6` + the merge cov writes now call `report::pct6` (provably behaviour-preserving — Phase-D goldens still byte-identical).
- **Steps 2–4 (`gpc.rs`, new ~430 ln):** `emit_gpc_dinucleotide` kernel (bottom-then-top, two `len<3` guards + classify-both gating the whole dinucleotide, per-strand NOMe CG-skip); non-overlapping `GC` scan (`j += 2`); `gpc_report_path`/`gpc_cov_path` (raw-`-o` base + `.NOMe` infix); `run_gpc` single + split mirroring `run_single`/`flush_split_chromosome` minus the summary and the uncovered pass; local `gpc_threshold = max(config.threshold, 1)` (config never mutated).
- **Step 6 (`lib.rs`):** `pub mod gpc;` + `if config.gc_context { gpc::run_gpc(config, &genome)?; }` after report → merge (order `:44/:58/:82`).
- **Step 7 (goldens + tests):** `tests/data/phase1/` (5 fixture genomes, 6 cov files, 15 per-mode Perl golden dirs) + `generate_goldens.sh` (full provenance — regenerates fixtures + goldens) + `tests/golden_phase1.rs` (18 dir-vs-Perl byte-identity tests covering V4–V21 + the raw-base divergence below). An out-of-tree scratch harness compared **all 16 modes** Rust≡Perl byte-for-byte before the goldens were committed.

**DEVIATION from the plan prose (matched to live Perl, not the §3.2.3 text):** the NOMe core **`.cov`** filename derives from the **un-stripped raw `-o`** (`output_raw`), NOT the stripped stem. `handle_filehandles` never suffix-strips `$cytosine_coverage_file` (`:96-101,:122`), only `$cytosine_report_file` (`:104-112`). For a plain `-o sample` the two bases coincide (so the §3.2.3 `{stem}.NOMe.CpG.cov` prose is right there); for `-o foo.CpG_report.txt --nome-seq` they diverge — report `foo.NOMe.CpG_report.txt` (stem) vs cov `foo.CpG_report.txt.NOMe.CpG.cov` (raw). **Verified against live Perl v0.25.1** (probe + the `nome_rawsuffix` golden). Implemented via `report::nome_cov_path` (raw base); the report still uses `report_name`'s stem/raw base-selection (reviewer A-M2 guidance, which was correct for the *report*). This is the only divergence from the plan; it makes the implementation MORE faithful to the byte-identity contract. The GpC report+cov already used the raw `-o` per §3.4 (no change there).
