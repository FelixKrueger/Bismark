...
11. **`--ffs` × `--nome-seq` interaction (rev 3, Phase-3 dual-review Critical).** Perl nests the core emit as `if ($tetra) { print CYT (10-col) } else { if ($nome) { print CYT (7-col); print CYTCOV } else { print CYT } }` (`:398-425`). So under `--ffs --nome-seq` the `$tetra` branch short-circuits **before** `print CYTCOV` → the NOMe `.NOMe.CpG.cov` companion is opened (because `$nome`) but **never written** (a 0-byte file); the NOMe core report is still the 10-col ffs line (ACG/TCG-filtered). The Rust must therefore gate the cov-companion write on **`nome && !ffs`** (NOT `nome` alone). This is the sibling-branch coupling the **Phase-1 plan §2/§10 flagged** ("keep them as sibling branches"; "`--ffs` + `--nome-seq` … a real but untested Perl combo — flag to Felix if Phase 3 needs it"). Pinned by V17.
...
| V17 | **`--ffs` × `--nome-seq`: NOMe `.cov` companion suppressed** (rev 3, the dual-review Critical) | `--ffs --nome-seq` on a covered-ACG/TCG-CpG fixture; diff the whole dir vs Perl golden | `.NOMe.CpG.cov` present but **0-byte** (Perl's `$tetra` short-circuits before `print CYTCOV`); the NOMe core report is the 10-col ffs line; GpC + summary unaffected — pins the `nome && !ffs` gate (Assumption 11) |
...
**Status: IMPLEMENTED.** Steps 1–7 complete; **168 crate tests green** (97 lib incl. 5 new `ffs_fields` + 1 `--ffs` CLI test; 9 Phase-3 goldens; 12 Phase-2 + 18 Phase-1 + 11 B + 7 C + 10 D + 4 sanity — **no regression**); `cargo fmt --check` + `clippy --all-targets -D warnings` clean. **Not yet committed; not yet dual-code-reviewed/plan-managed** (awaiting Felix's go-ahead).

**What was built (per §5, against the CURRENT post-Phase-1 `report.rs`):**
- **`report.rs`:** new `ffs_fields(seq, i, strand) -> (tetra, penta, hexa)` helper implementing the §3.2 offset table (forward `i/4`, `i/5`, `i-2/6` with the signed negative-wrap at i=0,1 gated by the numeric `len≥i+N` guards; reverse `revcomp` of `i-3/4`, `i-4/5`, `i-3/6` guarded `i≥3`/`i≥4`; N passed through). `emit_position` gained `ffs: bool` (threaded **alongside the existing `nome`**) and appends `\t{tetra}\t{penta}\t{hexa}` after `tri` when `ffs` (cols 1–7 byte-unchanged; computed only when `ffs`). `chromosome_report_bytes` passes `config.ffs`.
- **`cli.rs`:** dropped the `--ffs` `UnsupportedFlag` arm (ALL v1.x flags now supported — no flag rejected); `ffs: bool` in `ResolvedConfig` + constructor + help text; removed the obsolete `rejects_v1x_flags` test, added `ffs_resolves_and_composes` (composes with `--CX`/`--merge_CpGs`, no mutex). Added `ffs: false` to the gpc/drach/report in-test `ResolvedConfig` literals.
- **`tests/sanity.rs`:** removed the now-obsolete `unsupported_v1x_flag_is_rejected` probe (no v1.x flag is rejected any more; `missing_output_fails_with_clear_message` still covers "fails clearly").
- **No `merge.rs` change** — `parse_report_row` already tolerates the 10-col line (reads `f[0..6]`); V6 confirms `--ffs --merge_CpGs` merged cov == no-ffs.
- **Tests/goldens:** `tests/data/phase3_ffs/` (3 fixtures incl. the N-window + the uncovered-chrC main fixture, 7 Perl golden dirs) + `generate_goldens.sh`; `tests/golden_phase3.rs` (9 byte-identity tests, V5–V15). A 9-mode scratch sweep verified Rust≡Perl v0.25.1 byte-for-byte (CpG, CX, zero, split, gzip, merge-invariance, N-window) before the goldens were committed.

**DEVIATION from the plan's §4/§5 structure (sanctioned):** the plan suggested extending `extract` to return an `Extracted` struct (or 6-tuple). I instead added a **separate `ffs_fields` helper** and left `extract` untouched — the plan explicitly left the struct-vs-tuple choice to the implementer (§4), and a standalone helper is the same spirit with less churn, keeps `extract`'s shipped semantics intact, and makes the offset table directly unit-testable. Output is identical. The rev-1 post-Phase-1 rebase items (thread `ffs` alongside `nome`; `chromosome_report_bytes` now returns `(report, cov)`; narrow the rejection test; the `ffs:false` test-literal; the N-window "do not filter" note) were all handled.

**Post-review fix (rev 3, 2026-06-01) — the dual-code-review Critical.** Reviewer B caught that the initial implementation gated the NOMe `.cov` companion on `nome` alone, so `--ffs --nome-seq` wrote a non-empty `.NOMe.CpG.cov` while Perl writes a **0-byte** one (the `$tetra` branch short-circuits before `print CYTCOV` — `:398-425`). Confirmed against live Perl (Perl 0-byte vs Rust 72-byte). **Fix:** gate the cov write on `nome && !ffs` in `emit_position` (the file is still *created* — the writer opens on `config.nome` — just left empty under `--ffs`). Pinned by the new golden **V17** (`ffs_nome` dir; `.NOMe.CpG.cov` = 0 bytes, NOMe core report = 10-col). This was the sibling-branch interaction the Phase-1 plan §2/§10 had flagged for Phase 3 (Assumption 11). Reviewer A (APPROVE) had not exercised the combination — a clean illustration of why dual independent reviewers are required. 169 tests green after the fix.
...
- **rev 2** (2026-05-31): **IMPLEMENTED** (`/code-implementation`) — `ffs_fields` helper + `emit_position` append + `cli.rs` un-reject `--ffs`; 168 tests green; 9-mode scratch sweep + `golden_phase3.rs` (V5–V15) byte-identical to Perl. Sanctioned deviation: standalone `ffs_fields` helper (vs extending `extract`). See "Implementation notes".
- **rev 3** (2026-06-01): **dual code-review folded.** `CODE_REVIEW_A.md` APPROVE (0 Crit, 2 Low cosmetic); `CODE_REVIEW_B.md` **REQUEST-CHANGES — 1 Critical** (the `--ffs × --nome-seq` NOMe-`.cov`-suppression, confirmed against live Perl: Perl 0-byte vs Rust 72-byte). `COVERAGE.md` (plan-manager) COMPLETE (22 DONE, 1 sanctioned deviation) but did not cover the combo. **Fix:** gate the NOMe `.cov` write on `nome && !ffs` (`emit_position`); Assumption 11 + golden **V17** (`ffs_nome`) added. 169 tests green, fmt+clippy clean; all FFS modes incl. `--ffs --nome-seq` Rust≡Perl v0.25.1. The Critical was a sibling-branch interaction the Phase-1 plan had pre-flagged; A's APPROVE missed it (didn't test the combo) — vindicating the dual-reviewer requirement. Status → **verified; awaiting commit.**
