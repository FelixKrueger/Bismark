#!/bin/bash
# ===========================================================================
# run_scaling_rust.sh <reads> <tag> <mode>  — Rust bismark_rs -p scaling sweep
# mode ∈ {faithful_dir, comb_dir, faithful_nondir, comb1pass}
#   faithful_dir    --genome                                                          2 inst -> 2*p cores
#   comb_dir        --genome --combined_index                                         1 inst -> p cores   (directional combined; 10M reads, NO tagging)
#   faithful_nondir --genome --non_directional                                        4 inst -> 4*p cores
#   comb1pass       --genome --combined_index --non_directional --combined_index_single_pass  1 inst -> p cores (NON-DIR tagged, 20M)
# Reuses the process-tree RSS sampler. No concordance (perf only). -p {2,4,8,12,16}.
# ===========================================================================
set -uo pipefail
...
BR=/home/fkrueger/Bismark-bench/rust/target/release/bismark_rs
...
case "$MODE" in
  faithful_dir)    MARGS=(--genome "$GENOME"); MULT=2 ;;
  comb_dir)        MARGS=(--genome "$GENOME" --combined_index); MULT=1 ;;
  faithful_nondir) MARGS=(--genome "$GENOME" --non_directional); MULT=4 ;;
  comb1pass)       MARGS=(--genome "$GENOME" --combined_index --non_directional --combined_index_single_pass); MULT=1 ;;
...
run_cell(){
  ...
  local t0=$SECONDS
  /usr/bin/time -v -o "$tf" "$BR" "$@" "${pflag[@]}" --path_to_bowtie2 "$ENV" -o "$od" "$READS" > "$od/run.log" 2>&1 &
