...
| **C.1** | **Fix `drop_overlap` polarity reversal (PE 1.87× R2-call gap vs Perl). SPEC §7.4 rewrite + 4-LOC predicate flip + 8 test-assertion flips + 5 new regression-guard tests + smoke fixture rework.** Surfaced by Phase H partial-harness on 10M PE WGBS (2026-05-27). | ~4 LOC code + ~200 LOC tests + SPEC | 📝 **plan rev 1 — dual plan-reviewers complete (`PLAN_REVIEW_PHASE_C1_A/B.md`); 3 Critical + 7 Important folded; awaiting implementation trigger** | [#862](https://github.com/FelixKrueger/Bismark/issues/862) | `PHASE_C1_PLAN.md` rev 1 |
| G | `--bedGraph` + `--cytosine_report` subprocess chain | ~400 | ⏸ not started — **blocked on C.1** | — | — |
| H | Real-data byte-identity gate (10M + 55M PE WGBS) + CHANGELOG + version tag | ~200 test | ⏸ partial harness in place (`scripts/oxy_phase_h_smoke.sh`); full gate **blocked on C.1 + #863 + #864 + #865** | — | — |
