...
| G | `--bedGraph` + `--cytosine_report` subprocess chain | ~400 | ⏸ not started — **blocked on C.2** | — | — |
| H | Real-data byte-identity gate (10M + 55M PE WGBS) + CHANGELOG + version tag | ~200 test | ⏸ partial harness in place (`scripts/oxy_phase_h_smoke.sh`); full gate **blocked on C.2** | — | — |
