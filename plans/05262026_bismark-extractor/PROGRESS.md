...
| H (sub-gate 1, **SE**) | Byte-identity + speedup matrix harness (SE × 5-cell matrix × cross-N N-invariance check; reconciled with #872 in rev 1) | ~830 LOC actual (~660 LOC matrix driver + ~190 LOC checklist + ~80 LOC SPEC); +130 LOC rev 3 absorption (1 Crit + 4 High + 5 Med folded). No Rust code changes; 303-test baseline preserved. | ✅ **merged** (commit `651b7fd`, PR #873, 2026-05-28) | [#871](https://github.com/FelixKrueger/Bismark/issues/871) | `PHASE_H_SE_PLAN.md` rev 3 |
| H (sub-gate 1, **PE**) | Byte-identity + speedup matrix harness (PE × R1+R2 × multicore × overlap) — 5-cell matrix (D / r1_5p / r2_5p / r1r2_3p / overlap); 11,443 B M-bias baseline at (D, N=1) (rev 1 A-I5 dropped 875 B splitting-report absolute lock); mixed-metric differential (rev 1 A-O3): row count for `<D` cells + count-sum for `overlap`; new pre-flight overlap-fraction ≥ 80% gate (rev 1 A-I1); samtools-direct PE-ness assertion (rev 1 A-C1) | ~830 LOC est. (~750 LOC matrix driver with rev 1 hardening + ~50 LOC checklist PE-section populate + ~30 LOC SPEC + ~5 LOC PROGRESS). Pre-folds SE rev-3 hardening + 1 Critical + 9 Important + 8 Optional review findings. No Rust code changes. | 📝 **plan rev 1 — awaiting implementation trigger from Felix** (post dual-plan-review absorption; reviewer A NEEDS-REVISIONS / reviewer B APPROVE-WITH-NITS folded into rev 1) | [#872](https://github.com/FelixKrueger/Bismark/issues/872) | `PHASE_H_PE_PLAN.md` rev 1 |
