...
| **E** | **`--comprehensive` / `--merge_non_CpG` / `--yacht` output mode dispatch + `--gzip` + `--mbias_only` enablement + `mbias_only_silence` kernel param** | ~400 / ~530 actual | ✅ implementation complete (rev 1; 201 tests green; dual code-reviewers approve, plan-manager COMPLETE); ready to commit + PR | [#854](https://github.com/FelixKrueger/Bismark/issues/854) | `PHASE_E_PLAN.md` rev 1 |
