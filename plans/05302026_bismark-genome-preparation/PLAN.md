...
**Post-delta-review correction (rev-4, 2026-05-31) — a stop-hook-triggered focused dual review of the post-audit delta (`CODE_REVIEW_A2.md`/`B2.md`) caught a CONTRADICTION:** Reviewer B2 (on macOS) re-confirmed "case-insensitive, locale-independent"; Reviewer A2 traced `File::Glob.pm:69` and found `GLOB_NOCASE` is set *only* on Windows/VMS/OS2/DOS/RISCOS — the macOS case-fold is a Darwin-libc `GLOB_CSH` quirk, and **Linux-Perl/glibc sorts BYTEWISE**. Verified locally (macOS `glob<>`→folded but `bsd_glob(0)`→bytewise). **So the H1 "fix" was wrong for the Linux deployment target — it matched macOS-Perl and would have failed byte-identity on clusters/oxy + the mixed-case oracle on Linux CI.** Reverted `fasta_name_cmp` to **bytewise** (= the original SPEC premise, = Linux-Perl); kept M1 (orthogonal); the mixed-case oracle is now `#[cfg(target_os="linux")]`-gated; the unit test asserts bytewise; SPEC §8.1 re-corrected (rev-4). **Lesson (the project's own thesis): byte-identity must be verified on the TARGET platform (Linux), not dev (macOS).** Iteration `#5` — bytewise revert: 39 lib + 9 integration (mixed-case oracle gated out on macOS) pass; clippy/fmt clean.

**CI ground-truth correction (rev-5, 2026-05-31) — the bytewise revert was ALSO wrong; Linux CI settled it.** Pushed `79f8c76` (bytewise); Linux CI `cargo test` FAILED on `perl_vs_rust_mixed_case_glob_order` with the diff: Linux-Perl emitted `chr1, Chr10, CHR2, Scaffold_a, scaffold_b` (**case-insensitive/folded**) vs Rust's bytewise `CHR2, Chr10, Scaffold_a, chr1, scaffold_b`. **Root cause of A2's error:** Perl's `glob`/`<>` does NOT call libc `glob(3)` — it uses its own bundled `File::Glob::bsd_glob` (csh_glob), which folds case on **both** Linux and macOS; A2's `GLOB_NOCASE`-not-set → "Linux bytewise" inference was a red herring. **Restored case-insensitive `fasta_name_cmp` `(lowercased, raw)` (= rev-3), ungated the oracle** (matches Perl on both platforms). SPEC §8.1 corrected (rev-5). Iteration `#6` — case-insensitive restore: re-verify + push, expect Linux CI `cargo test` green (the mixed-case oracle now matches Linux-Perl). **Meta-lesson:** three flips (bytewise→CI→bytewise→CI); the dev platform (macOS) could not adjudicate the glob contract — only the target platform's CI could. Should have pushed to CI before trusting either review's platform claim.
