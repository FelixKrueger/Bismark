...
**Phase E — COMPLETE (2026-05-31, oxy `dockyard-oxy-0`).** Branch `rust/genomeprep-phase-e` (off the post-#913 iron-chancellor):
- **E1** — `tests/byte_identity_real_data.rs` `#[ignore]` harness (env-var-gated, fake indexer, streams the comparison for GB-scale files).
- **E2 (#910 gate) — PASSED.** Perl v0.25.1 vs the Rust binary (built on oxy) on copies of two real genomes; converted CT/GA FASTA compared byte-for-byte: **E. coli `NC_010473.fa.gz`** (4.75 MB, gzipped) MFA + `--single_fasta` → byte-identical (md5 match; also validates the gzip-input path vs Perl `gunzip -c`); **human GRCh38** (3.15 GB CT) MFA → byte-identical (validates streaming at scale). Fake `bowtie2-build` for both (gate = converted FASTA, not the index).
- **E3 (#911)** — crate `CHANGELOG.md` written (Validation section carries the oxy results); `README.md` shipped in #913; mkdocs `docs/bismark/genome_preparation.md` reviewed + accurate (convention: user docs stay Perl-focused; the `_rs` binary is a drop-in).

**After the Phase E PR merges:** close #910/#911 + epic #912 (board → Done); file the 2 follow-up issues (rammap-at-aligner; `--genomic_composition`). The `--genomic_composition` feature itself stays deferred (accepted-and-ignored).
