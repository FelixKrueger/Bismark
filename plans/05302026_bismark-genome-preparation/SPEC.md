...
1. **Glob sort order parity (load-bearing) — CASE-INSENSITIVE (CI-verified).** Perl `<*.fa>` returns a sorted list; this fixes the MFA concatenation order and the indexer `file_list`. **RESOLVED (rev-5, confirmed on Linux CI): the order is case-insensitive (ASCII fold), with raw bytes as tiebreak.** Perl's `glob`/`<>` does **not** use the platform libc `glob(3)`; it uses its own bundled `File::Glob::bsd_glob` (csh_glob path), which **case-folds on BOTH Linux and macOS** (not via `GLOB_NOCASE` — set only on Windows/VMS/… — but as csh_glob's own ordering). **Empirically pinned on Linux CI** (the deployment target): files `{chr1, Chr10, CHR2, Scaffold_a, scaffold_b}.fa` → Perl emits `chr1, Chr10, CHR2, Scaffold_a, scaffold_b` (folded), NOT the bytewise `CHR2, Chr10, Scaffold_a, chr1, scaffold_b`. So `discovery::fasta_name_cmp` sorts `(ascii-lowercased, raw bytes)`; the `perl_vs_rust_mixed_case_glob_order` oracle is **ungated** (matches Perl on both platforms). Still digit-vs-letter lexical (`chr1, chr10, chr2`). Real genomes use all-lowercase `chrN.fa` where fold == bytewise, so it never bites them regardless.
   - **History (three flips, settled by CI):** original SPEC = bytewise; rev-3 = case-insensitive (code-review B, macOS); rev-4 = bytewise (code-review A2 traced `File::Glob.pm` NOCASE + assumed Perl uses glibc `glob(3)` → "Linux bytewise"); **rev-5 = case-insensitive, after Linux CI proved Perl folds there too — A2's premise was wrong because Perl bundles its own `bsd_glob`, it does not call libc `glob(3)`.** Lesson: when reviewers contradict on a platform fact, let the target platform's CI run the oracle rather than pick the more elaborate argument.
